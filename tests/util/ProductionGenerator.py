...
class ProductionGenerator(Generator):
    ...
    def createConfigDict(self, lvl=""):
        ...
        production_config["ProductType"] = self.addProductType(
            lvl, ast.literal_eval(production_config["Process"]["settings"])
        )
        machineDict = self.addMachine(
            lvl, ast.literal_eval(production_config["Process"]["settings"])
        )
        production_config["Machine"] = machineDict

        production_config["Task"] = self.addTask(
            lvl, ast.literal_eval(production_config["ProductType"]["settings"])
        )
        if lvl == "lvl1":
            production_config["Transporter"] = self.addTransporter(
                lvl, machineDict["queue_dict"]
            )
            production_config["Process"] = self.adjustProcess(
                production_config["Machine"], production_config["Process"]
            )
        ...
        production_config["DeadLock_Queue"] = self.addSpecialQueue(
            lvl, "DeadLock_Queue"
        )
    ...
    def addProcess(self, lvl):
        ...
        if lvl == "lvl1":
            ...
            for i in range(numberOfProcesses):
                ...
                deviation = self.randomDistribution.randint(0, 5)
                config["settings"].append(
                    {"id": i, "mean": mean, "deviation": deviation, "type": "normal"}
                )
        ...
        for i in range(0, numberOfProductType):
            ...
            for b in range(numberOfPaths):
                productTypeList.append(
                    [
                        self.randomDistribution.randint(0, len(processConfigList) - 1)
                        for b in range(self.randomDistribution.randint(2, 5))
                    ]
                )
    ...
    def addMachine(self, lvl, flatProcessList):
        ...
        for i in range(self.randomDistribution.randint(1, 4)):
            for process_id in range(len(flatProcessList)):
                if self.randomDistribution.random() > 0.5:
                    distributeProcess[
                        math.floor(
                            self.randomDistribution.random() * number_of_machines
                        )
                    ].append(process_id)
        ...
        if lvl == "lvl1":
            ...
            numberOfQueues = (int)(
                abs(self.randomDistribution.normal(number_of_machines, 2))
            ) + 3
            for b in range(numberOfQueues):
                queueConfig = {
                    "queue_id": self.queue_id,
                    "number_of_positions": self.randomDistribution.randint(1, 5),
                    "location": [self.randomDistribution.randint(0, 10), 5, 0],
                }
                queueConfig["add_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
                queueConfig["remove_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
            ...
            for i in range(number_of_machines):
                ...
                machineConfig = {
                    "machine_id": i,
                    "waiting_time": self.randomDistribution.randint(1, 5),
                ...
                for from_process in distributeProcess[i]:
                    for to_process in distributeProcess[i]:
                        if from_process == to_process:
                            machineConfig["set_up"].append(
                                {
                                    "start": from_process,
                                    "end": to_process,
                                    "mean": round(self.randomDistribution.random(), 1),
                                    "deviation": round(
                                        self.randomDistribution.random(), 1
                                    ),
                                    "type": "normal",
                                }
                            )
                ...
                numberOfInputQueues = (
                    int(abs(self.randomDistribution.normal(0, 0.5))) + 1
                )
                numberOfOutputQueues = (
                    int(abs(self.randomDistribution.normal(0, 0.5))) + 1
                )
                ...
                for input in range(numberOfInputQueues):
                    machineConfig["queue_input_id"].append(
                        queue_dict[
                            self.randomDistribution.randint(0, numberOfQueues - 1)
                        ]["queue_id"]
                    )
                ...
                machineConfig["queue_output_id"] = list(
                    set(machineConfig["queue_output_id"])
                )
                machineConfig["queue_input_id"] = list(
                    set(machineConfig["queue_input_id"])
                )
            ...
            for i in range(number_of_machines):
                machineDict = {
                    "number_of_positions": self.randomDistribution.randint(1, 6),
                    "process": distributeProcess[i],
                    "location": [location_x, location_y, 0],
                }
                if (i + 1) % int(math.sqrt(number_of_machines)) == 0:
                ...
                machineDict["add_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
                machineDict["remove_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
                ...
                if self.randomDistribution.random() > 0.5:
                    for process in processList:
                        if process["id"] == processMachineID:
                            ...
                            mean = (
                                processList[i]["default"]["mean"]
                                * (10 + self.randomDistribution.random())
                                / 10
                            )
                            deviation = (
                                processList[i]["default"]["deviation"]
                                * (10 + self.randomDistribution.random())
                                / 10
                            )
            ...
            for type in ["start", "logging", "end"]:
                if type == "start" or type == "end":
                    ...
                    config["settings"].append(
                        {
                            "product_type": productType["id"],
                            "number_of_parts": self.randomDistribution.randint(10, 15)
                            * 10,
                            "type": type,
                        }
                    )
    ...
    def addTransporter(self, lvl, machine_queue_list=None):
        ...
        if lvl == "lvl3":
            ...
            config["start_location"] = [
                {"id": i, "location": [i * 10, 0, 0]}
                for i in range(numberStartPositions)
            ]
            config["settings"] = []
            for i in range(numberOfTransporter):
                transporterSetting = {}
                transporterSetting["number_of_positions"] = int(
                    (self.randomDistribution.random() * 5 + 1)
                )
                transporterSetting["location_id"] = (
                    math.ceil(self.randomDistribution.random() * numberStartPositions)
                    % numberStartPositions
                )
                transporterSetting["speed"] = self.randomDistribution.randint(2, 5)
                transporterSetting["add_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
                transporterSetting["remove_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                    "deviation": round(self.randomDistribution.random(), 2),
                    "type": "normal",
                }
            ...
            for i in range(numberOfTransporter):
                ...
                transporterSetting["speed"] = self.randomDistribution.randint(2, 5)
                transporterSetting["waiting_time"] = self.randomDistribution.randint(
                    1, 5
                )
                transporterSetting["add_time"] = {
                    "mean": round(self.randomDistribution.random() * 2, 2),
                ...
                if routeType == 0 or i == 0:
                    ...
                    numberOfQueues = int(
                        abs(self.randomDistribution.normal(2, len(machine_queue_list)))
                    )
                    queueList = []
                    for b in range(numberOfQueues):
                        queuePosition = self.randomDistribution.randint(
                            0, len(machine_queue_list)
                        ) % len(machine_queue_list)
                        queueList.append(machine_queue_list[queuePosition]["queue_id"])
                    queueList = list(set(queueList))
                    transporterSetting["route"] = {
                        "type": "restricted",
                        "queue_list": queueList,
                    }
                else:
                    numberOfQueues = self.randomDistribution.randint(
                        2, len(machine_queue_list)
                    )
                    queueList = []
                    for b in range(numberOfQueues):
                        queueList.append(
                            machine_queue_list[
                                self.randomDistribution.randint(
                                    0, len(machine_queue_list) - 1
                                )
                                % len(machine_queue_list)
                            ]["queue_id"]
                        )
                ...
                config["settings"] = [
                    {
                        ...
                        "add_time": {
                            "mean": round(self.randomDistribution.random(), 2),
                            "deviation": round(self.randomDistribution.random(), 2),
                            "type": "normal",
                        },
                        "remove_time": {
    ...
    def addDefect(self, lvl):
        ...
        if lvl == "lvl3":
            transporter_defect_possible = bool(
                round(self.randomDistribution.random() + 0.2)
            )
            machine_defect_possible = bool(
                round(self.randomDistribution.random() + 0.2)
            )
            ...
            config["transporter_normal"] = {
                "defect": {
                    "type": "normal",
                    "mean": self.randomDistribution.randint(0, 3) * 500,
                    "deviation": self.randomDistribution.randint(0, 5) * 50,
                },
                "repair": {
                    "mean": self.randomDistribution.randint(0, 3) * 10,
            ...
            transporter_defect_possible = bool(
                round(self.randomDistribution.random() + 0.3)
            )
            machine_defect_possible = bool(
                round(self.randomDistribution.random() + 0.3)
            )
            ...
            config["transporter_random"] = {
                "type": "random",
                "min": 0,
                "max": numberTransporterDefects - 1,
            }
            transporter_normal = []
            for i in range(numberTransporterDefects):
                transporter_normal.append(
                    {
                        "defect_type": str(i),
                        "defect": {
                            "type": "normal",
            ...
            config["machine_random"] = {
                "type": "random",
                "min": 0,
                "max": numberMachineDefects - 1,
            }
            machine_normal = []
            for i in range(numberMachineDefects):
                machine_normal.append(
                    {
                        "defect_type": str(i),
                        "defect": {
                            "type": "normal",
