    ...
    def __call__(self, *args):
        ...
        try:
            if type == Label.Transporter.name or type == "all":
                response_dict[Label.Transporter.name] = (
                    self.flaskApp.simCore.transport.transformToDict("all")
                )
            ...
            if type == Label.Machine.name or type == "all":
                response_dict[Label.Machine.name] = (
                    self.flaskApp.simCore.machine.transformToDict("all")
                )
            ...
            if type == Label.StartQueue.name or type == "all":
                ...
                for queue_onto in self.flaskApp.simCore.central.start_queue_list:
                    response_dict[Label.StartQueue.name] = (
                        self.flaskApp.simCore.queue.transformToDict(queue_onto.name)
                    )
            ...
            if type == "TransporterQueue" or type == "all":
                ...
                for (
                    queue_onto_name
                ) in self.flaskApp.simCore.central.queue_to_transporter.keys():
                    response_dict["TransporterQueue"] = (
                        self.flaskApp.simCore.queue.transformToDict(queue_onto_name)
                    )
                        ...
                        json.dumps(
                            {
                                "error": "400",
                                "name": "component/id",
                                "description": "type is not defined or programmed: "
                                + type,
                            }
                        )
                    },
