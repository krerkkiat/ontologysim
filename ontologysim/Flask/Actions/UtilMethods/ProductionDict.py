...
class ProductionDict:
    ...
    def getProductionDict(cls, simCore):
        ...
        for key, value in response_dict[Label.Transporter.name].items():
            ...
            response_dict[Label.Transporter.name][key]["kpi"] = {
                "AUTTp": attKPI,
                "AUSTp": astKPI,
            }
        ...
        for key, value in response_dict[Label.Machine.name].items():
            if stateStorage != None:
                if key in stateStorage.machine.keys():
                    ...
                    if key in stateStorage.lastProcess.keys():
                        response_dict[Label.Machine.name][key]["last_process"] = (
                            stateStorage.lastProcess[key]
                        )
                else:
                    response_dict[Label.Machine.name][key]["state"] = {}
            else:
            ...
            response_dict[Label.Machine.name][key]["kpi"] = {
                "APTp": aptKPI,
                "ASTp": astKPI,
            }
        ...
        for queue_onto in simCore.central.machine_queue_list:
            response_dict["MachineQueue"][queue_onto.name] = (
                simCore.queue.transformToDict(queue_onto.name)[queue_onto.name]
            )
        ...
        for machine_onto in simCore.central.machine_list:
            for queue_onto in machine_onto.has_for_queue_process:
                response_dict["ProcessQueue"][queue_onto.name] = (
                    simCore.queue.transformToDict(queue_onto.name)[queue_onto.name]
                )

        response_dict["TransporterQueue"] = {}
        for queue_onto_name in simCore.central.queue_to_transporter.keys():
            response_dict["TransporterQueue"][queue_onto_name] = (
                simCore.queue.transformToDict(queue_onto_name)[queue_onto_name]
            )

        response_dict["Position"] = {}
        for position_onto in simCore.onto.search(type=simCore.central.position_class):
            response_dict["Position"][position_onto.name] = (
                simCore.position.transformToDict(position_onto.name)[position_onto.name]
            )

        response_dict["ProductType"] = {}
        for productTypeOnto in simCore.onto.search(
            type=simCore.central.product_type_class
        ):
            response_dict["ProductType"][productTypeOnto.name] = (
                simCore.product_type.transformToDict(productTypeOnto.name)[
                    productTypeOnto.name
                ]
            )

        response_dict["Product"] = {}
        for productOnto in simCore.onto.search(type=simCore.central.product_class):
            response_dict["Product"][productOnto.name] = (
                simCore.product.transformToDict(productOnto.name)[productOnto.name]
            )

        response_dict["Task"] = {}
        for taskOnto in simCore.onto.search(type=simCore.central.task_class):
            response_dict["Task"][taskOnto.name] = simCore.task.transformToDict(
                taskOnto.name
            )[taskOnto.name]
