...
class StateStorage(object):
    ...
    def changeState(self, event_onto):
        ...
        if event_type == Evaluate_Enum.Machine.value:
            ...
            self.machine[machine_onto.name] = {
                "state": event_type,
                "timeDiff": event_onto.time_diff,
            }
            ...
            self.transporter[transport_onto.name] = {
                "state": event_type,
                "timeDiff": event_onto.time_diff,
            }
            ...
            self.machine[machine_onto.name] = {
                "state": event_type,
                "timeDiff": event_onto.time_diff,
                "process": process_onto.is_prodprocess_of_process[0].process_id,
            }

            self.rememberProcess[machine_onto.name] = (
                process_onto.is_prodprocess_of_process[0].process_id
            )
            ...
            if event_onto.additional_type == Queue_Enum.RemoveFromTransporter.value:
                ...
                self.transporter[transporter_onto.name] = {
                    "state": event_type,
                    "timeDiff": event_onto.time_diff,
                }
                ...
                if machine_onto.name in self.rememberProcess.keys():
                    self.lastProcess[machine_onto.name] = self.rememberProcess[
                        machine_onto.name
                    ]
            ...
            newQueueLocation = self.simCore.location.location_queue_dict["location"][
                new_location_onto.name
            ]
            oldQueueLocation = self.simCore.location.location_queue_dict["location"][
                old_location.name
            ]
            ...
            self.transporter[transport_onto.name] = {
                "state": event_type,
                "timeDiff": event_onto.time_diff,
                "newLocation": newQueueLocation,
                "oldLocation": oldQueueLocation,
            }
