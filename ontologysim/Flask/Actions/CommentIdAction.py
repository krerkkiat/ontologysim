                ...
                if label == "id not found":
                    ...
                    if label == Label.Machine.name:
                        response_dict[Label.Machine.name] = (
                            self.flaskApp.simCore.machine.transformToDict(id)
                        )
                    elif label == Label.Transporter.name:
                        response_dict[Label.Transporter.name] = (
                            self.flaskApp.simCore.transport.transformToDict(id)
                        )
                    elif (
                        label == Label.Queue.name
                        or label == Label.StartQueue.name
                        or label == Label.EndQueue.name
                        or label == Label.DeadlockQueue.name
                    ):
                        if label == Label.Queue.name:
                            response_dict[Label.Queue.name] = (
                                self.flaskApp.simCore.queue.transformToDict(id)
                            )
