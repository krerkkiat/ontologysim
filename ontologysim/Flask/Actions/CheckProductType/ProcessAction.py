        ...
        if "list" in requestDict.keys():
            ...
            if not self.checkInputProcessList(process_list):
                self.response = self.response400BadRequest(
                    "process list is not correct"
                )
            else:
                allProcesses = list(
                    set([item for sublist in process_list for item in sublist])
                )
                process_config = []
                for process in allProcesses:
                    process_config.append(
                        {"id": process, "mean": 1, "deviation": 1, "type": "normal"}
                    )
                ...
                initializer.s.product_type.createProductType(
                    {"id": 0, "config": process_list}, {}
                )

                product_type_config = (
                    initializer.s.product_type.getCompleteProductTypePath(
                        initializer.s.onto.search(
                            type=initializer.s.central.product_type_class
                        )[0].name
                    )
                )
