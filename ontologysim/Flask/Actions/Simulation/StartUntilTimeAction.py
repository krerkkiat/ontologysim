        ...
        if (
            ...
            if isDragDropSelected and isDefaultSelected:
                ...
                if not requestDict["onlyLastEvent"]:
                    while (
                        self.flaskApp.simCore.getCurrentTimestep() < time
                        and self.flaskApp.simCore.run
                    ):
                        event_list.append(
                    ...
                    production_dict = ProductionDict.getProductionDict(
                        self.flaskApp.simCore
                    )
                    ...
                    self.response = Response(
                        ...
                        response={
                            json.dumps(
                                {
                                    "eventOntoList": event_list,
                                    "productionDict": production_dict,
                                    "simulationFinished": not self.flaskApp.simCore.run,
                                }
                            )
                        },
