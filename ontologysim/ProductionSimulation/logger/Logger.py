...
from ontologysim.ProductionSimulation.logger.ProductAnalyseLogger import (
    ProductAnalyseLogger,
)
from ontologysim.ProductionSimulation.logger.QueueFillLevelLogger import (
    QueueFillLevelLogger,
)
from ontologysim.ProductionSimulation.logger.SimLogger import SimLogger
from ontologysim.ProductionSimulation.logger.TransporterDistributionLogger import (
    TransporterDistributionLogger,
)
from ontologysim.ProductionSimulation.logger.TransporterLocationLogger import (
    TransporterLocationLogger,
)
...
class Logger:
    ...
    def __init__(self, simCore):
        ...
        self.transform_event_type_in_log_type[Queue_Enum.Change.value] = [
            self.queueFillLevelLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[Queue_Enum.AddToTransporter.value] = [
            self.queueFillLevelLogger,
            self.transporterLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[
            Queue_Enum.RemoveFromTransporter.value
        ] = [
            self.queueFillLevelLogger,
            self.transporterLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[Queue_Enum.StartProcess.value] = [
            self.queueFillLevelLogger,
            self.machineLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[Queue_Enum.EndProcess.value] = [
            self.queueFillLevelLogger,
            self.machineLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[Queue_Enum.Default.value] = [
            self.queueFillLevelLogger,
            self.productAnalyseLogger,
        ]
        self.transform_event_type_in_log_type[Queue_Enum.StartOfProduction.value] = [
            self.productAnalyseLogger
        ]
        self.transform_event_type_in_log_type[
            Queue_Enum.RemoveFromTransporterDeadlock.value
        ] = [
            self.queueFillLevelLogger,
            self.productAnalyseLogger,
            self.transporterLogger,
        ...
        self.transform_event_type_in_log_type[
            Queue_Enum.StartProcessStayBlocked.value
        ] = [self.queueFillLevelLogger, self.machineLogger, self.productAnalyseLogger]

        self.transform_event_type_in_log_type[Transporter_Enum.Defect.value] = [
            self.transporterLogger
        ]
        self.transform_event_type_in_log_type[Transporter_Enum.Wait.value] = [
            self.transporterLogger
        ]
        self.transform_event_type_in_log_type[Transporter_Enum.Transport.value] = [
            self.transporterLogger,
            self.transporterDistributionLogger,
            self.transporterLocationLogger,
        ]

        self.transform_event_type_in_log_type[OrderRelease_Enum.Release.value] = [
            self.simLogger,
            self.productAnalyseLogger,
        ]

        self.transform_event_type_in_log_type[Machine_Enum.Wait.value] = [
            self.machineLogger
        ]
        self.transform_event_type_in_log_type[Machine_Enum.Defect.value] = [
            self.machineLogger
        ]
        self.transform_event_type_in_log_type[Machine_Enum.SetUp.value] = [
            self.machineLogger
        ]
        self.transform_event_type_in_log_type[Machine_Enum.Process.value] = [
            self.machineLogger,
            self.productAnalyseLogger,
        ]

        self.transform_event_type_in_log_type[Evaluate_Enum.ProductFinished.value] = [
            self.productAnalyseLogger,
            self.simLogger,
        ]
    ...
    def setStartLogger(self, time):
        ...
        self.simLogger.start_logging_multiple = math.floor(
            self.start_time_logging / self.time_intervall
        )
        ...
        self.plot.start_logging_multiple = math.floor(
            self.start_time_logging / min_logger_interval
        )
    ...
    def initLogger(self, log_conf):
        ...
        if (
            Logger_Enum.SimLogger.value in summary_event_list
            and Logger_Enum.ProductLogger.value not in summary_event_list
            and Logger_Enum.MachineLogger.value not in summary_event_list
        ):
            raise Exception(
                "add " + Logger_Enum.ProductLogger.value + " to log_summary list"
            )
        if (
            Logger_Enum.SimLogger.value in time_event_list
            and Logger_Enum.ProductLogger.value not in time_event_list
            and Logger_Enum.MachineLogger.value not in time_event_list
        ):
            raise Exception(
                "add "
                + Logger_Enum.ProductLogger.value
                + " and "
                + Logger_Enum.MachineLogger.value
                + " to log_time list"
            )

        log_type_dict = {}
        for element in Logger_Enum:
            log_type_dict[element.value] = self.getLoggerType(
                element.value, summary_event_list, time_event_list
            )
    ...
    def create_output(self):
        ...
        self.queueFillLevelLogger.save(
            self.output_path, Folder_name.queue.value, "queue_logger"
        )
        self.machineLogger.save(
            self.output_path, Folder_name.machine.value, "machine_logger"
        )
        self.transporterLogger.save(
            self.output_path, Folder_name.transporter.value, "transporter_logger"
        )
        self.simLogger.save(self.output_path, Folder_name.sim.value, "sim_logger")
        self.productAnalyseLogger.save(
            self.output_path, Folder_name.product.value, "product_logger"
        )
        self.transporterDistributionLogger.save(
            self.output_path,
            Folder_name.transporter_distribution.value,
            "transporter_distribution_logger",
        )
        self.transporterLocationLogger.save(
            self.output_path,
            Folder_name.transporter_location.value,
            "transporter_location_logger",
        )
        ...
        if self.save_config["database"]:
            ...
            if self.simCore.run_as_api:
                ...
                user = (
                    ...
                    .filter_by(userName=userJSON["userName"])
                    .first()
                )
            ...
            if (
                "all" in self.productAnalyseLogger.number_of_products.keys()
                and "all" in self.simLogger.summarized_kpis
            ):
                simulationFact = SimulationFacts(
                    numberOfParts=self.productAnalyseLogger.number_of_products["all"],
                    loggingTime=self.simLogger.summarized_kpis["all"]["logging_time"],
                ...
                simulationFact = SimulationFacts(
                    numberOfParts=None,
                    loggingTime=(
                        self.simCore.getCurrentTimestep() - self.start_time_logging
                    ),
                    simulationTime=self.simCore.getCurrentTimestep(),
                )
    ...
    def addIni(self, path):
        ...
        if self.ini_config["addini"]:
            ...
            onlyfiles = [
                ...
                if os.path.isfile(os.path.join(ini_dir, f))
            ]
