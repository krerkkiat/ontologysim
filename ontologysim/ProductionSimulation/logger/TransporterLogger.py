...
class TransporterLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        transporter_list = [
            transporter
            for transporter in self.logger.simCore.onto.search(
                type=self.logger.simCore.central.transporter_class
            )
        ]
        ...
        if not self.isNotLogging():
            for transporter in transporter_list:
                ...
                self.last_value[transporter_name]["TTFp"] = (
    ...
    def addElement(self, dict_element):
        ...
        if self.isTimeLogging():
            if len(self.time_kpis["time"]) < math.ceil(
                ...
                self.addNewTimeElement(
                    [
                        transport.name
                        for transport in self.logger.simCore.onto.search(
                            type=self.logger.simCore.central.transporter_class
                        )
                    ],
                    self.kpi_list,
        ...
        if event_type == Transporter_Enum.Defect.value:
            ...
            if self.isSummaryLogging():
                ...
                self.summarized_kpis[transporter_name]["TTFp"] += (
                    triggered_defect_time - self.last_value[transporter_name]["TTFp"]
                )
                self.summarized_kpis[transporter_name]["TTRp"] += (
                    time - triggered_defect_time
                )
                self.summarized_kpis[transporter_name]["CMTp"] += repair_time
                self.summarized_kpis[transporter_name]["ADOTp"] = self.summarized_kpis[
                    transporter_name
                ]["TTRp"]
            ...
            if self.isTimeLogging():
                ...
                self.addTimeElement(
                    transporter_name, ["TTRp"], time, time - triggered_defect_time
                )
                self.addTimeElement(transporter_name, ["CMTp"], time, repair_time)

                self.addTimeSumElement(
                    transporter_name, ["FE"], time, time - triggered_defect_time
                )
            ...
            transporter_name = self.logger.simCore.central.queue_to_transporter[
                queue_name
            ]
        ...
        if self.isTimeLogging():
            for transporter_name in transporter_list:
                self.addTimeElement(
                    ...
                    ["TTFp"],
                    time,
                    time - self.last_value[transporter_name]["TTFp"],
                )

            for (
                k,
                v,
            ) in self.summarized_kpis.items():
        ...
        if self.isSummaryLogging():
            for transporter_name in transporter_list:
                ...
                for k1, v1 in self.summarized_kpis["all"].items():
                    ...
                    if k1 != "FE":
                        self.summarized_kpis[transporter_name][k1] /= (
                            time - self.logger.start_time_logging
                        )

            for k1, v1 in self.summarized_kpis["all"].items():
                if k1 != "FE":
                    self.summarized_kpis["all"][k1] /= (
                        time - self.logger.start_time_logging
                    )
    ...
    def save_to_database(self):
        ...
        if self.isTimeLogging():
            ...
            for i in range(0, len(self.time_kpis["time"])):
                ...
                transporterTimeKPIvalue = TransporterTimeKPIValue(
                    **input, time=self.time_kpis["time"][i]
                )
                transporterTimeKPI.transporterTimeKPIValue.append(
                    transporterTimeKPIvalue
                )
            ...
            for transporter in transporter_list:
                ...
                for i in range(0, len(self.time_kpis["time"])):
                    ...
                    self.logger.dataBase.session.add(transporterTimeKPIvalue)
                self.logger.simulationRunDB.transporterTimeKPI.append(
                    transporterTimeKPI
                )
                self.logger.dataBase.session.add(transporterTimeKPI)
        ...
        if self.isSummaryLogging():
            ...
            for transporter in transporter_list:
                transporterKPI = TransporterKPI(
                    name=transporter, **self.summarized_kpis[transporter]
                )
                self.logger.simulationRunDB.transporterKPI.append(transporterKPI)
                self.logger.dataBase.session.add(transporterKPI)
