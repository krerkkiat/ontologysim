...
class SimLogger(SubLogger.SubLogger):
    ...
    def addElement(self, dict_element):
        ...
        if self.isTimeLogging():
            if len(self.time_kpis["time"]) < math.ceil(
                ...
                self.addNewTimeElement([], self.basic_kpi_list, time)
                self.addNewTimeElement([], ["WIP"], time)
                ...
                for i in range(time_index - time_len, -1, -1):
                    ...
                    self.time_kpis["all"]["logging_time"].append(
        ...
        if self.isSummaryLogging():
            self.summarized_kpis["all"]["WIP"] += (self.last_WIP) * (
                time - self.last_time
            )
            ...
            for k, v in split_time_dict.items():
                for kpi_key in kpi_key_list:
                    ...
                    self.time_kpis["all"][kpi_key][k] += (
                        time_intervall_diff / self.logger.time_intervall * wip
                    )
        ...
        if self.isTimeLogging():
            if self.product_update and self.machine_update:
                for update_index in self.update_list:
                    if (
                        self.logger.productAnalyseLogger.time_kpis["all"]["AOET"][
                            update_index
                        ]
                        > 0
                    ):
                        self.time_kpis["all"]["AR"][update_index] = (
                            self.logger.machineLogger.time_kpis["all"]["AUBTp"][
                                update_index
                            ]
                            / self.logger.productAnalyseLogger.time_kpis["all"]["AOET"][
                                update_index
                        ...
                        self.time_kpis["all"]["PR"][update_index] = (
                            self.logger.machineLogger.time_kpis["all"]["APTp"][
                                update_index
                            ]
                            / self.logger.productAnalyseLogger.time_kpis["all"]["AOET"][
                                update_index
    ...
    def finale_evaluate(self, time):
        ...
        if self.isSummaryLogging():
            ...
            self.summarized_kpis["all"]["AR"] = (
                self.logger.machineLogger.summarized_kpis["all"]["AUBTp"]
                / self.logger.productAnalyseLogger.summarized_kpis["all"]["AOET"]
            )
            self.summarized_kpis["all"]["PR"] = (
                self.logger.machineLogger.summarized_kpis["all"]["APTp"]
                / self.logger.productAnalyseLogger.summarized_kpis["all"]["AOET"]
            )
            self.summarized_kpis["all"]["logging_time"] = (
    ...
    def finale_evaluate_summary_api(self, time, machineSummary, productSummary):
        ...
        summarized_data["all"]["AR"] = (
            machineSummary["all"]["AUBTp"] / productSummary["all"]["AOET"]
        )
        summarized_data["all"]["PR"] = (
            machineSummary["all"]["APTp"] / productSummary["all"]["AOET"]
        )
    ...
    def save_to_database(self):
        ...
        if self.isTimeLogging():
            ...
            for i in range(0, len(self.time_kpis["time"])):
                ...
                simulationTimeKPIvalue = SimulationTimeKPIValue(
