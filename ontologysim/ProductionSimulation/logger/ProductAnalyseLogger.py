...
class ProductAnalyseLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        product_type_list = [
            product_type
            for product_type in self.logger.simCore.onto.search(
                ...
                if Label.EndQueue.value not in queue.name:
                    for position in queue.has_for_position:
                        if (
                            ...
                            and len(position.has_for_product) > 0
                        ):
    ...
    def addElement(self, dict_element):
        ...
        if event_type == Machine_Enum.Process.value:
            ...
            self.product_kpis[product_name]["AQMTp"] += (
                time - self.last_value[product_name]["machine_queue_time"] - time_diff
            )
            ...
            if self.isTimeLogging():
                ...
                if aqmt_time > 0:
                    self.addTimeElement(
                        ...
                        time,
                        time
                        - self.last_value[product_name]["machine_queue_time"]
                        - time_diff,
                    )
            ...
            event_type == Queue_Enum.RemoveFromTransporter.value
            or event_type == Queue_Enum.RemoveFromTransporterDeadlock.value
        ):
            dict_element = self.checkStartTimeLogging(dict_element)
            time_diff = dict_element["time_diff"]
            time = dict_element["event_onto_time"]
            ...
            if (
                time
                - self.last_value[product_name]["transporter_queue_time"]
                - time_diff
                > 0
            ):
                self.product_kpis[product_name]["ATTp"] += (
                    time
                    - self.last_value[product_name]["transporter_queue_time"]
                    - time_diff
                )
            ...
            number_of_products_before = self.wip[
                self.product_kpis[product_name]["ProductType"]
            ]["number_of_products"][-1]

            self.summarized_kpis[self.product_kpis[product_name]["ProductType"]][
                "WIP"
            ] += number_of_products_before * (
                time
                - self.wip[self.product_kpis[product_name]["ProductType"]][
                    "start_time"
                ][-1]
            )
            ...
            self.wip[self.product_kpis[product_name]["ProductType"]][
                "number_of_products"
            ].append(number_of_products_before - 1)
            ...
            event_type == Queue_Enum.Change.value
            or event_type == Queue_Enum.Default.value
        ):
            ...
            number_of_products_before = self.wip[product_type_name][
                "number_of_products"
            ][-1]

            self.summarized_kpis[product_type_name]["WIP"] += (
                number_of_products_before
            ...
            self.summarized_kpis[product_type_name]["WIP"] += (
                number_of_products_before * (time - self.wip["all"]["start_time"][-1])
            )
            self.wip["all"]["number_of_products"].append(
                number_of_products_before + number_of_products
            )
            self.wip["all"]["start_time"].append(time)
    ...
    def addTimeElement(self, object_name, kpi_key_list, time, time_diff):
        ...
        for k, v in split_time_dict.items():
            number_of_instances = self.calculateWIPTimeDiff(
                v["time_diff"], v["end_time"], object_name
            )
            if self.number_of_product_types != 1:
                number_of_instances_all = self.calculateWIPTimeDiff(
                    v["time_diff"], v["end_time"], "all"
                )
            else:
        ...
        for b in range(len(self.wip[object_name]["start_time"]) - 1, -1, -1):
            if end_time_element_instances - self.wip[object_name]["start_time"][b] > 0:
                ...
                if current_time_element_instances > 0:
                    ...
                    number_of_instances += (
                        self.wip[object_name]["number_of_products"][b]
                        * time_difference
                        / time_diff
                    )
                ...
                if (
                    b != len(self.wip[object_name]["start_time"]) - 1
    ...
    def calculateWIP(self, end_time_intervall):
        ...
        if self.isTimeLogging():
            if end_time_intervall % self.logger.time_intervall != 0:
                start_time_intervall = (
                    math.floor(end_time_intervall / self.logger.time_intervall)
                    * self.logger.time_intervall
                )
                index = int(
                    start_time_intervall / self.logger.time_intervall
                    - self.start_logging_multiple
                )
                for k, v in self.wip.items():
                    self.time_kpis[k]["WIP"][index] = (
                        self.calculateWIPTimeDiff(
                            end_time_intervall - start_time_intervall,
                            end_time_intervall,
                            k,
                        )
                        * (end_time_intervall - start_time_intervall)
                ...
                for k, v in self.wip.items():
                    self.time_kpis[k]["WIP"][index] = self.calculateWIPTimeDiff(
                        self.logger.time_intervall, end_time_intervall, k
                    )
            ...
            for update_index in self.update_list:
                for k, v in self.number_of_products_time.items():
                    if len(self.number_of_products_time[k]) < update_index:
                        for i in range(
                            len(self.number_of_products_time[k]) - 1, update_index + 1
                        ):
                            self.number_of_products_time[k].append(0)
                    # print(len(self.number_of_products_time[k]))
                    self.time_kpis[k]["NrP"][update_index] = (
                        self.number_of_products_time[k][update_index]
                    )
                    self.time_kpis[k]["TR"][update_index] = (
                        self.number_of_products_time[k][update_index]
                        / self.logger.time_intervall
                    )
                    if self.number_of_products_time[k][update_index] > 0:
                        self.time_kpis[k]["AOET"][update_index] = (
                            self.time_kpis[k]["AOET"][update_index]
                            / self.number_of_products_time[k][update_index]
                        )
    ...
    def finale_evaluate(self, time):
        ...
        if number_of_products > logging_number_of_products:
            product_kpis_keys = sorted(
                [
                    int(str(product).replace(Label.Product.value, ""))
                    for product in self.product_kpis.keys()
                ]
            )
                ...
                for k1, v1 in v.items():
                    if k1 != "start_time" and k1 != "end_time":
                        if k1 != "ProductType" and k1 != "AOET" and k1 != "WIP":
                            self.summarized_kpis[self.product_kpis[k]["ProductType"]][
                                k1
                            ] += self.product_kpis[k][k1] / self.product_kpis[k]["AOET"]
                        elif k1 == "AOET":
                            self.summarized_kpis[self.product_kpis[k]["ProductType"]][
                                k1
            ...
            for product_type_name in [
                ...
                for k, v in self.summarized_kpis[product_type_name].items():
                    if k == "WIP":
                        ...
                        self.summarized_kpis["all"]["WIP"] += self.summarized_kpis[
                            product_type_name
                        ]["WIP"]
                    elif k != "ProductType":
                        self.summarized_kpis["all"][k] += (
                            self.summarized_kpis[product_type_name][k]
                ...
                self.summarized_kpis[product_type_name]["TR"] = self.number_of_products[
                    product_type_name
                ] / (time - self.logger.start_time_logging)

                self.summarized_kpis[product_type_name]["NrP"] = (
                    self.number_of_products[product_type_name]
                )
            self.summarized_kpis["all"]["NrP"] = self.number_of_products["all"]

            # all produces parts/time
            self.summarized_kpis["all"]["TR"] = self.summarized_kpis["all"]["NrP"] / (
                time - self.logger.start_time_logging
            )
    ...
    def finale_evaluate_summary_api(self, time):
        ...
        for k, v in self.product_kpis.items():
            for k1, v1 in v.items():
                if k1 != "start_time" and k1 != "end_time":
                    if k1 != "ProductType" and k1 != "AOET" and k1 != "WIP":
                        summarized_data[self.product_kpis[k]["ProductType"]][k1] += (
                            self.product_kpis[k][k1] / self.product_kpis[k]["AOET"]
                        )
                    elif k1 == "AOET":
                        summarized_data[self.product_kpis[k]["ProductType"]][k1] += (
        ...
        for product_type_name in [
            ...
            for k, v in summarized_data[product_type_name].items():
                if k == "WIP":
                    summarized_data[product_type_name]["WIP"] = summarized_data[
                        product_type_name
                    ]["WIP"] / (time - self.logger.start_time_logging)
                    summarized_data["all"]["WIP"] += summarized_data[product_type_name][
                        "WIP"
                    ]
                elif k != "ProductType":
                    summarized_data["all"][k] += (
                        summarized_data[product_type_name][k]
                        / self.number_of_products["all"]
                    )
                    if self.number_of_products[product_type_name] != 0:
                        summarized_data[product_type_name][k] = (
            ...
            summarized_data[product_type_name]["TR"] = self.number_of_products[
                product_type_name
            ] / (time - self.logger.start_time_logging)

            summarized_data[product_type_name]["NrP"] = self.number_of_products[
                product_type_name
            ]
            ...
            for k, v in self.time_kpis.items():
                if k != "time":
                    with open(
                        ...
                        wr = csv.writer(
                            product_time_logger, delimiter=";", quoting=csv.QUOTE_ALL
                        )
            ...
            for k, v in self.time_kpis.items():
                if k != "time":
                    ...
                    for i in range(len(self.time_kpis["time"])):
                        ...
                        productTimeKPIvalue = ProductTimeKPIValue(
            ...
            for kpi_key in self.time_kpis["all"].keys():
                ...
                for object_name in object_list:
                    ...
                    if (
                        ...
                        and self.time_kpis[object_name][kpi_key][i] > 1
                    ):
                        print(
