...
class TransporterLocationLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        self.location_list = [
            location.name
            for location in self.logger.simCore.onto.search(
                type=self.logger.simCore.central.location_class
            )
        ]
    ...
    def addElement(self, dict_element):
        ...
        if self.isTimeLogging():
            if (
                ...
                < math.ceil(time / self.logger.time_intervall)
                - math.floor(
    ...
    def evaluate(self, time):
        ...
        if self.isTimeLogging():
            self.time_kpis["time"].append(
                (time_multiple - 1) * self.logger.time_intervall
            )
        ...
        for location_string in self.location_list:
            ...
            for transport, location_dict in self.current_sum.items():
                if self.current_number[transport] != 0:
                    kpi_value = (
                        self.current_sum[transport][location_string]
                ...
                transport_count += 1
            if self.isTimeLogging():
                self.time_kpis["all"][location_string].append(
                    transport_sum / transport_count
                )
            ...
            for location_string in self.location_list:
                ...
                for transport, location_dict in self.current_sum.items():
                    ...
                    if self.number_transport[transport] != 0:
                        self.summarized_kpis[transport][location_string] = (
                            self.summarized_kpis[transport][location_string] / self.pbt
                        )
                    else:
        ...
        if self.isSummaryLogging():
            ...
            for location_string in self.location_list:
                ...
                for transport, location_dict in self.current_sum.items():
                    ...
                    if self.number_transport[transport] != 0:
                        summarized_data[transport][location_string] = (
                            summarized_data[transport][location_string] / self.pbt
                        )
                    else:
                        ...
                        for k, v in self.time_kpis[key].items():
                            transporterLocationTimeKPIvalue = (
                                TransporterLocationTimeKPIValue(
                                    value=v[i],
                                    location=k,
                                    time=self.time_kpis["time"][i],
                                )
                            )
                            transporterLocationTimeKPI.transporterLocationTimeKPIValue.append(
                                transporterLocationTimeKPIvalue
                            )
                            self.logger.dataBase.session.add(
                                transporterLocationTimeKPIvalue
                            )
                    self.logger.simulationRunDB.transporterLocationTimeKPI.append(
                        transporterLocationTimeKPI
                    )
        ...
        if self.isSummaryLogging():
            for key, value in self.summarized_kpis.items():
                for k, v in self.summarized_kpis[key].items():
                    transporterLocationKPI = TransporterLocationKPI(
                        name=key, location=k, value=v
                    )
                    self.logger.dataBase.session.add(transporterLocationKPI)
                    self.logger.simulationRunDB.transporterLocationKPI.append(
                        transporterLocationKPI
                    )
