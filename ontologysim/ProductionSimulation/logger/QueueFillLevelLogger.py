...
class QueueFillLevelLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        self.last_time_intervall = (
            math.floor(
                self.logger.simCore.getCurrentTimestep() / self.logger.time_intervall
            )
    ...
    def addElement(self, dict_element):
        ...
        if Label.EndQueue.value not in queue_onto_name:
            ...
            self.kpi_current_time_slot_dict[old_queue_onto_name][
                old_current_size + 1
            ] += (
                time - self.current_time_blocked[old_queue_onto_name]
            ) / self.logger.time_intervall
            self.kpi_current_time_slot_dict[queue_onto_name][current_size - 1] += (
                time - self.current_time_blocked[queue_onto_name]
            ) / self.logger.time_intervall
    ...
    def evaluate(self, time):
        ...
        if not self.isNotLogging():
            ...
            for queue, position_dict in self.kpi_current_time_slot_dict.items():
                time_diff = (
                    time_multiple * self.logger.time_intervall
                    - self.current_time_blocked[queue]
                )
                ...
                self.kpi_current_time_slot_dict[queue][
                    self.current_size_blocked[queue]
                ] += (
                    time_diff
                    / self.logger.time_intervall
                    * self.current_size_blocked[queue]
    ...
    def finale_evaluate(self, time):
        ...
        if self.isSummaryLogging():
            ...
            count = 0
            for queue, position_dict in self.kpi_current_time_slot_dict.items():
                self.summarized_kpis[queue]["FillLevel"] = (
                    self.summarized_kpis[queue]["FillLevel"]
    ...
    def finale_evaluate_summary_api(self, time):
        ...
        if not self.isNotLogging():
            ...
            for queue, position_dict in kpiCurrentTimeSlot.items():
                time_diff = (
                    time_multiple * self.logger.time_intervall
                    - current_time_blocked[queue]
                )

                current_time_blocked[queue] = time_multiple * self.logger.time_intervall
                kpiCurrentTimeSlot[queue][current_size_blocked[queue]] += (
                    time_diff / self.logger.time_intervall * current_size_blocked[queue]
                )
                count = 0
                sum = 0
                count_queue += 1
            ...
            count = 0
            for queue, position_dict in kpiCurrentTimeSlot.items():
                summarized_data[queue]["FillLevel"] = (
                    summarized_data[queue]["FillLevel"]
                    / (time - self.logger.start_time_logging)
                    * self.logger.time_intervall
                )
            ...
            for key, value in self.time_kpis.items():
                if key != "time":
                    ...
                    for i in range(0, len(self.time_kpis["time"])):
                        ...
                        queueTimeKPIvalue = QueueTimeKPIValue(
