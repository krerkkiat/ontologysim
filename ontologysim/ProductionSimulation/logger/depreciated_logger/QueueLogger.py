...
class QueueLogger(Logger):
    ...
    def init_dict(self):
        ...
        queue_list = [
            queue
            for queue in self.queue.simCore.central.queue_list
    ...
    def add_element(
        self, old_queue_onto_name, time, old_current_size, queue_onto_name, current_size
    ):
        """
    ...
    def evaluate(self, time):
        ...
        for queue, position_dict in self.kpi_current_time_slot_dict.items():
            time_diff = (
                time_multiple * self.time_intervall - self.current_time_blocked[queue]
            )
            ...
            ] += time_diff / self.time_intervall * self.current_size_blocked[queue]
            count = 0
            sum = 0
            count_queue += 1
    ...
    def finale_evaluate(self, time):
        ...
        """
        for queue, position_dict in self.kpi_current_time_slot_dict.items():
            self.kpi_dict[queue] = self.kpi_dict[queue] / math.ceil(
                time / self.time_intervall
            )
    ...
    def insert_db(self):
        ...
        for k, v in self.kpi_dict.items():
            insert_string += (
                ...
                + str(v)
                + ", "
                + str(self.queue.simCore.currentRunID)
                + ")"
            )
