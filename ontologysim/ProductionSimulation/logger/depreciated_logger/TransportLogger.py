...
class TransporterLogger(Logger):
    ...
    def init_dict(self):
        ...
        transport_list = [
            transport
            for transport in self.tranpsort.simCore.onto.search(
                type=self.tranpsort.simCore.central.transporter_class
            )
        ]
        # tod_o programm start queue len(queue.is_transp_queue_of)==0 and
        location_list = [
            queue.has_for_queue_location.__getitem__(0)
            for queue in self.tranpsort.simCore.central.queue_list
    ...
    def evaluate(self, time):
        ...
        for location_string in self.location_from_to_list:
            ...
            self.kpi_time_dict["all"][location_string].append(
    ...
    def finale_evaluate(self, time):
        ...
        for location_string in self.location_from_to_list:
            ...
            count = 0
            for transport, location_dict in self.current_sum.items():
                self.kpi_dict[transport][location_string] = (
                    self.kpi_dict[transport][location_string]
    ...
    def save_to_csv(self, type):
        ...
        with open(self.path_csv, type, newline="") as order_logger:
            ...
            header.extend(
                [
                    str(transport) + " " + str(location)
                    for transport, time_dict in self.kpi_dict.items()
                    for location, time in time_dict.items()
                ]
            )
            ...
            for i in range(len(self.kpi_time_dict["time"])):
                ...
                erg.extend(
                    [
                        self.kpi_time_dict[transport][location][i]
                        for transport, time_dict in self.kpi_dict.items()
                        for location, time in time_dict.items()
                    ]
                )
        ...
        for k, v in self.kpi_dict.items():
            for k1, v1 in v.items():
                insert_string += (
                    ...
                    + "'"
                    + ", "
                    + str(self.tranpsort.simCore.currentRunID)
                    + ")"
                )
