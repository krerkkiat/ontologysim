...
class MachineLogger(Logger):
    ...
    def init_dict(self):
        ...
        machine_list = [
            machine
            for machine in self.machine.simCore.onto.search(
                type=self.machine.simCore.central.machine_class
            )
        ]
    ...
    def add_element(self, machine_onto_name, time, new_event_name):
        ...
        self.kpi_current_time_slot_dict[machine_onto_name][old_event] += (
            time - self.current_time[machine_onto_name]
        ) / self.time_intervall
    ...
    def evaluate(self, time):
        ...
        for event in Machine_Enum:
            ...
            for machine, event_dict in self.kpi_current_time_slot_dict.items():
                time_diff = (
                    time_multiple * self.time_intervall - self.current_time[machine]
                )
                self.current_time[machine] = time_multiple * self.time_intervall
                self.kpi_current_time_slot_dict[machine][
                    self.current_event[machine]
                ] += time_diff / self.time_intervall
    ...
    def finale_evaluate(self, time):
        ...
        for event in Machine_Enum:
            ...
            count = 0
            for machine, event_dict in self.kpi_current_time_slot_dict.items():
                self.kpi_dict[machine][event.value] = (
                    self.kpi_dict[machine][event.value] / time
                )
    ...
    def save_to_csv(self, type):
        ...
        with open(self.path_csv, type, newline="") as order_logger:
            ...
            header.extend(
                [
                    str(machine) + " " + str(event)
                    for machine, time_dict in self.kpi_dict.items()
                    for event, time in time_dict.items()
                ]
            )
            ...
            for i in range(len(self.kpi_time_dict["time"])):
                ...
                erg.extend(
                    [
                        self.kpi_time_dict[machine][event][i]
                        for machine, time_dict in self.kpi_dict.items()
                        for event, time in time_dict.items()
                    ]
                )
        ...
        for k, v in self.kpi_dict.items():
            for k1, v1 in v.items():
                insert_string += (
                    ...
                    + str(v1)
                    + ", "
                    + str(machine_enum_dict[k1])
                    + ", "
                    + str(self.machine.simCore.currentRunID)
                    + ")"
                )
