...
class ProductFinishedLogger(Logger):
    ...
    def add_product(
        self, product_name, start_time, finished_time, product_type, time_diff
    ):
        """
        ...
        self.product_list.append(
            [product_name, start_time, finished_time, product_type.name, time_diff]
        )
    ...
    def save_to_csv(self, type):
        ...
        with open(self.path_csv[:-4] + "_dict.csv", type, newline="") as order_logger:
            ...
            for i in range(len(self.kpi_time_dict["time"])):
                ...
                erg.extend(
                    [
                        self.kpi_time_dict[product_type][i]
                        for product_type, time_dict in self.kpi_dict.items()
                    ]
                )
    ...
    def init_dict(self):
        ...
        product_type_list = [
            product_type
            for product_type in self.simCore.onto.search(
    ...
    def evaluate(self, time):
        ...
        for product_type, kpi in self.kpi_current_time_slot_dict.items():
            ...
            if self.number_current_time_slot_dict[product_type] > 0:
                kpi_value = (
                    self.kpi_current_time_slot_dict[product_type]
                    / self.number_current_time_slot_dict[product_type]
                )
            if kpi_value > 0:
        ...
        self.kpi_time_dict["all"].append(
            sum_time_product_type / count_time_product_type
        )
    ...
    def finale_evaluate(self, time):
        ...
        for product_type, kpi in self.number_dict.items():
            if self.kpi_dict[product_type] != 0:
                self.kpi_dict[product_type] = (
                    self.kpi_dict[product_type] / self.number_dict[product_type]
                )
            else:
                self.kpi_dict[product_type] = 0
        self.kpi_dict["all"] = sum(self.kpi_dict.values()) / sum(
            self.number_dict.values()
        )
    ...
    def insert_db(self):
        ...
        while run and i + 1 != product_finished_len:
            ...
            insert_string += str(tuple(self.product_list[end_i]))
            ...
            for b in range(start_b, end_b):
                ...
                for k, v in self.kpi_dict.items():
                    insert_string += (
                        ...
                        + "'"
                        + ", "
                        + str(self.kpi_time_dict[k][b])
                        + ", "
                        + str(self.kpi_time_dict["time"][b])
                        + ", "
                        + str(self.simCore.currentRunID)
                        + ")"
                    )
                    if (
                        i != len_dict * len_sub_dict - 1
                        and b < start_b + number_of_time_steps
                    ):
                        insert_string += " , "
                        i += 1
