...
class Plot:
        ...
        for setting in self.setting_dict:
            ...
            if listEmpty:
                ...
                if setting["kpi"] in self.number_time_kpis:
                    ...
                    raise Exception(
                        setting["kpi"] + " not defined in y_lookup_tabel.ini"
                    )
                number_of_axis += 1
    ...
    def startPlot(self, time):
        ...
        for logger_dict in self.logger_data:
            logger_dict["x"][0] = (
                logger_dict["sub_logger"].start_logging_multiple
                * logger_dict["sub_logger"].logger.time_intervall
            )
            (logger_dict["line"],) = logger_dict["axis"].plot(
                [],
                [],
                color_list[i],
                lw=3,
                label=logger_dict["type"]
                + "_"
                + logger_dict["object_name"]
                + "_"
                + logger_dict["kpi"],
            )
    ...
    def update_plot(self, time):
        ...
        """
        if (
            self.logger.plot.plot_activated
            and self.logger.start_logging
            and self.last_time_index
            < math.floor(time / self.time_intervall) - self.start_logging_multiple - 1
        ):
            for logger_dict in self.logger_data:
                self.update_figure_with_new_value(
                    logger_dict,
                    logger_dict["x"][-1]
                    + 1 * logger_dict["sub_logger"].logger.time_intervall,
            ...
            self.last_time_index = (
                math.floor(time / self.time_intervall) - self.start_logging_multiple
            )
