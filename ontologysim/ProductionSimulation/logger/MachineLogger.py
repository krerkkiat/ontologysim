...
class MachineLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        machine_list = [
            machine
            for machine in self.logger.simCore.onto.search(
                type=self.logger.simCore.central.machine_class
            )
        ]
        ...
        if not self.isNotLogging():
            for machine in machine_list:
                ...
                self.last_value[machine_name]["TTF"] = (
                    self.logger.simCore.getCurrentTimestep()
                )
    ...
    def setTTFlastValue(self, start_logging_time):
        ...
        if not self.isNotLogging():
            machine_list = [
                machine.name for machine in self.logger.simCore.central.machine_list
            ]
    ...
    def update_basic_kpis(self):
        ...
        if self.isTimeLogging():
            ...
            for update_index in list(set(self.update_list)):
                ...
                for machine_name in machine_list:
                    self.time_kpis[machine_name]["AUBTp"][update_index] = (
                        self.time_kpis[machine_name]["AUPTp"][update_index]
                        + self.time_kpis[machine_name]["ADOTp"][update_index]
                    )
                    self.time_kpis[machine_name]["A"][update_index] = (
                        self.time_kpis[machine_name]["APTp"][update_index] / pbt * pbt
                    )
                    self.time_kpis[machine_name]["AE"][update_index] = (
                        self.time_kpis[machine_name]["AUBTp"][update_index] / pbt * pbt
                    )
                    self.time_kpis["all"]["AE"][update_index] += (
                        self.time_kpis[machine_name]["AE"][update_index]
                        / self.number_of_instances
                    )
                    self.time_kpis["all"]["A"][update_index] += (
                        self.time_kpis[machine_name]["A"][update_index]
                        / self.number_of_instances
                    )
                    self.time_kpis["all"]["AUBTp"][update_index] += (
                        self.time_kpis[machine_name]["AUBTp"][update_index]
                        / self.number_of_instances
                    )
                    if (
                        self.time_kpis[machine_name]["APTp"][update_index]
                        + self.time_kpis[machine_name]["ADOTp"][update_index]
                    ) > 0:
                        self.time_kpis[machine_name]["TE"][update_index] = (
                            self.time_kpis[machine_name]["APTp"][update_index]
                            / (
                                self.time_kpis[machine_name]["APTp"][update_index]
                                + self.time_kpis[machine_name]["ADOTp"][update_index]
                        ...
                        self.time_kpis["all"]["TE"][update_index] += (
                            self.time_kpis[machine_name]["TE"][update_index]
                            / self.number_of_instances
                        )

                        self.time_kpis[machine_name]["E"][update_index] = (
                            self.time_kpis[machine_name]["PRIp"][update_index]
                            / self.time_kpis[machine_name]["APTp"][update_index]
                        )
                        self.time_kpis["all"]["E"][update_index] += (
                            self.time_kpis[machine_name]["E"][update_index]
                            / self.number_of_instances
                        )

                        self.time_kpis[machine_name]["OEE"][update_index] = (
                            self.time_kpis[machine_name]["A"][update_index]
                            * self.time_kpis[machine_name]["E"][update_index]
                        )
                        self.time_kpis["all"]["OEE"][update_index] += (
                            self.time_kpis[machine_name]["OEE"][update_index]
                            / self.number_of_instances
                        )

                    if self.time_kpis[machine_name]["AUBTp"][update_index] > 0:
                        self.time_kpis[machine_name]["UE"][update_index] = (
                            self.time_kpis[machine_name]["APTp"][update_index]
                            / self.time_kpis[machine_name]["AUBTp"][update_index]
                        )
                        self.time_kpis["all"]["UE"][update_index] += (
                            self.time_kpis[machine_name]["UE"][update_index]
                            / self.number_of_instances
                        )
                    if self.time_kpis[machine_name]["AUPTp"][update_index] > 0:
                        self.time_kpis[machine_name]["SeRp"][update_index] = (
                            self.time_kpis[machine_name]["AUSTp"][update_index]
                            / self.time_kpis[machine_name]["AUPTp"][update_index]
                        )
                        self.time_kpis["all"]["SeRp"][update_index] += (
                            self.time_kpis[machine_name]["SeRp"][update_index]
                            / self.number_of_instances
                        )

                    if self.time_kpis[machine_name]["PBTp"][update_index] > 0:
                        self.time_kpis[machine_name]["NEE"][update_index] = (
                            self.time_kpis[machine_name]["AUPTp"][update_index]
                            / self.time_kpis[machine_name]["PBTp"][update_index]
                            * self.time_kpis[machine_name]["E"][update_index]
                        )
                        self.time_kpis["all"]["NEE"][update_index] += (
                            self.time_kpis[machine_name]["NEE"][update_index]
                            / self.number_of_instances
                        )

            self.logger.simLogger.update_list.extend(
                [update_index for update_index in list(set(self.update_list))]
            )
            self.update_list = []
    ...
    def addElement(self, dict_element):
        ...
        if self.isTimeLogging():
            if len(self.time_kpis["time"]) < math.ceil(
                time / self.logger.time_intervall
            ) - math.floor(self.logger.start_time_logging / self.logger.time_intervall):
                time_index = int(
                ...
                for i in range(time_index - time_len, -1, -1):
                    self.time_kpis["time"].append(
                        (time_multiple - i) * self.logger.time_intervall
                    )
        ...
        if event_type == Machine_Enum.Defect.value:
            ...
            if self.isSummaryLogging():
                ...
                self.summarized_kpis[machine_name]["TTFp"] += (
                    triggered_defect_time - self.last_value[machine_name]["TTFp"]
                )
                self.summarized_kpis[machine_name]["TTRp"] += (
                    time - triggered_defect_time
                )
                self.summarized_kpis[machine_name]["PBTp"] += (
                    time - triggered_defect_time
                )
                self.summarized_kpis[machine_name]["CMTp"] += repair_time
                self.summarized_kpis[machine_name]["ADOTp"] = self.summarized_kpis[
                    machine_name
                ]["TTRp"]
            if self.isTimeLogging():
                self.addTimeElement(
                    machine_name,
                    ["TTFp"],
                    triggered_defect_time,
                    triggered_defect_time - self.last_value[machine_name]["TTFp"],
                )
                self.addTimeElement(
                    machine_name, ["TTRp", "PBTp"], time, time - triggered_defect_time
                )
                self.addTimeElement(machine_name, ["CMTp"], time, repair_time)
                self.addTimeSumElement(
                    machine_name, ["FE"], time, time - triggered_defect_time
                )
            ...
            if self.isTimeLogging():
                self.addTimeElement(
                    machine_name, ["AUSTp", "AUPTp", "PBTp"], time, time_diff
                )
            ...
            if self.isTimeLogging():
                self.addTimeElement(
                    machine_name, ["APTp", "AUPTp", "PBTp"], time, time_diff
                )
                self.addTimeElement(machine_name, ["PRIp"], time, meanProcessTime)

        elif (
            event_type == Queue_Enum.StartProcess.value
            or event_type == Queue_Enum.StartProcessStayBlocked.value
        ):
            dict_element = self.checkStartTimeLogging(dict_element)
            time_diff = dict_element["time_diff"]
        ...
        if not self.isNotLogging():
            ...
            if self.isSummaryLogging():
                for machine_name in machine_list:
                    ...
                    self.summarized_kpis[machine_name]["TTFp"] += (
                        time - self.last_value[machine_name]["TTFp"]
                    )

                    for k1, v1 in self.summarized_kpis["all"].items():
                        self.summarized_kpis["all"][k1] += self.summarized_kpis[
                            machine_name
                        ][k1]
            ...
            if self.isTimeLogging():
                ...
                for k, v in self.summarized_kpis.items():
                    ...
                    self.time_kpis[k]["AUBTp"] = (
                        self.time_kpis[k]["AUPTp"] + self.time_kpis[k]["ADOTp"]
                    )
            ...
            if self.isSummaryLogging():
                ...
                for machine_name in machine_list:
                    self.summarized_kpis[machine_name]["A"] = (
                        self.summarized_kpis[machine_name]["APTp"] / self.pbt * self.pbt
                    )
                    self.summarized_kpis[machine_name]["AE"] = (
                        self.summarized_kpis[machine_name]["AUBTp"]
                        / self.pbt
                        * self.pbt
                    )
                    ...
                    if self.summarized_kpis[machine_name]["APTp"] > 0:
                        self.summarized_kpis[machine_name]["E"] = (
                            self.summarized_kpis[machine_name]["PRIp"]
                            / self.summarized_kpis[machine_name]["APTp"]
                        )

                        self.summarized_kpis[machine_name]["OEE"] = (
                            self.summarized_kpis[machine_name]["A"]
                            * self.summarized_kpis[machine_name]["E"]
                        )

                    if self.summarized_kpis[machine_name]["PBTp"] > 0:
                        self.summarized_kpis[machine_name]["NEE"] = (
                            self.summarized_kpis[machine_name]["AUPTp"]
                            / self.summarized_kpis[machine_name]["PBTp"]
                            * self.summarized_kpis[machine_name]["E"]
                        )
                    ...
                    if (
                        ...
                        self.summarized_kpis[machine_name]["TE"] = self.summarized_kpis[
                            machine_name
                        ]["APTp"] / (
                            self.summarized_kpis[machine_name]["APTp"]
                            + self.summarized_kpis[machine_name]["ADOTp"]
                    ...
                    if self.summarized_kpis[machine_name]["AUBTp"] > 0:
                        self.summarized_kpis[machine_name]["UE"] = (
                            self.summarized_kpis[machine_name]["APTp"]
                            / self.summarized_kpis[machine_name]["AUBTp"]
                        )
                    else:
                    ...
                    if self.summarized_kpis[machine_name]["AUPTp"] > 0:
                        self.summarized_kpis[machine_name]["SeRp"] = (
                            self.summarized_kpis[machine_name]["AUSTp"]
                            / self.summarized_kpis[machine_name]["AUPTp"]
                        )
                    ...
                    self.summarized_kpis["all"]["A"] += self.summarized_kpis[
                        machine_name
                    ]["A"]
                    self.summarized_kpis["all"]["AE"] += self.summarized_kpis[
                        machine_name
                    ]["AE"]
                    self.summarized_kpis["all"]["TE"] += self.summarized_kpis[
                        machine_name
                    ]["TE"]
                    self.summarized_kpis["all"]["UE"] += self.summarized_kpis[
                        machine_name
                    ]["UE"]
                    self.summarized_kpis["all"]["SeRp"] += self.summarized_kpis[
                        machine_name
                    ]["SeRp"]
                    self.summarized_kpis["all"]["E"] += self.summarized_kpis[
                        machine_name
                    ]["E"]
                    self.summarized_kpis["all"]["OEE"] += self.summarized_kpis[
                        machine_name
                    ]["OEE"]
                    self.summarized_kpis["all"]["NEE"] += self.summarized_kpis[
                        machine_name
                    ]["NEE"]
    ...
    def save_to_database(self):
        ...
        if self.isTimeLogging():
            ...
            for i in range(0, len(self.time_kpis["time"])):
                ...
                machineTimeKPIvalue = MachineTimeKPIValue(
                    **input, time=self.time_kpis["time"][i]
                )
    ...
    def finale_evaluate_summary_api(self, time):
        ...
        for machine_name in machine_list:
            summarized_data[machine_name]["ADOTp"] = summarized_data[machine_name][
                "TTRp"
            ]
            ...
            summarized_data[machine_name]["TTFp"] += (
                time - self.last_value[machine_name]["TTFp"]
            )
        ...
        for machine_name in machine_list:
            summarized_data[machine_name]["A"] = (
                summarized_data[machine_name]["APTp"] / self.pbt * self.pbt
            )
            summarized_data[machine_name]["AE"] = (
                summarized_data[machine_name]["AUBTp"] / self.pbt * self.pbt
            )

            if summarized_data[machine_name]["APTp"] > 0:
                summarized_data[machine_name]["E"] = (
                    summarized_data[machine_name]["PRIp"]
                    / summarized_data[machine_name]["APTp"]
                )

                summarized_data[machine_name]["OEE"] = (
                    summarized_data[machine_name]["A"]
                    * summarized_data[machine_name]["E"]
                )

            if summarized_data[machine_name]["PBTp"] > 0:
                summarized_data[machine_name]["NEE"] = (
                    summarized_data[machine_name]["AUPTp"]
            ...
            if (
                ...
                summarized_data[machine_name]["TE"] = summarized_data[machine_name][
                    "APTp"
                ] / (
                    summarized_data[machine_name]["APTp"]
                    + summarized_data[machine_name]["ADOTp"]
            ...
            if summarized_data[machine_name]["AUBTp"] > 0:
                summarized_data[machine_name]["UE"] = (
                    summarized_data[machine_name]["APTp"]
                    / summarized_data[machine_name]["AUBTp"]
                )
            else:
            ...
            if summarized_data[machine_name]["AUPTp"] > 0:
                summarized_data[machine_name]["SeRp"] = (
                    summarized_data[machine_name]["AUSTp"]
                    / summarized_data[machine_name]["AUPTp"]
                )
