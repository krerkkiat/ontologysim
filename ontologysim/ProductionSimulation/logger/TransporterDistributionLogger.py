...
class TransporterDistributionLogger(SubLogger.SubLogger):
    ...
    def initSubLogger(self, log_type_dict):
        ...
        location_list = [
            queue.has_for_queue_location.__getitem__(0)
        ...
        for location in location_list:
            for location1 in location_list:
                self.location_from_to_list.append(
    ...
    def save_to_database(self):
        ...
        if self.isTimeLogging():
            for key, value in self.time_kpis.items():
                if key != "time":
                    transporterDistributionTimeKPI = TransporterDistributionTimeKPI(
                        name=key
                    )
                    ...
                    for i in range(0, len(self.time_kpis["time"])):
                        for k, v in self.time_kpis[key].items():
                            ...
                            transporterDistributionTimeKPI.transporterDistributionTimeKPIValue.append(
                                transporterDistributionTimeKPIvalue
                            )
                            self.logger.dataBase.session.add(
                                transporterDistributionTimeKPIvalue
                            )
                    self.logger.simulationRunDB.transporterDistributionTimeKPI.append(
                        transporterDistributionTimeKPI
                    )
        ...
        if self.isSummaryLogging():
            for key, value in self.summarized_kpis.items():
                for k, v in self.summarized_kpis[key].items():
                    transporterDistributionKPI = TransporterDistributionKPI(
                        name=key, fromToLocation=k, value=v
                    )
                    self.logger.dataBase.session.add(transporterDistributionKPI)
                    self.logger.simulationRunDB.transporterDistributionKPI.append(
                        transporterDistributionKPI
                    )
