...
class TransformProductionIni:
    ...
    def transform_ini(self, production_ini={}):
        ...
        if type == "lvl3":
            ...
            output_ini["Machine"] = self.transform_machine(
                type,
                production_ini["Machine"],
                defaultValuesConfig,
                production_ini["ChangeTime"],
            )
            output_ini["Start_Queue"] = self.transform_start_queue(
                type, {}, defaultValuesConfig["Start_Queue"]
            )
            output_ini["End_Queue"] = self.transform_end_queue(
                type, {}, defaultValuesConfig["End_Queue"]
            )
            output_ini["DeadLock_Queue"] = self.transform_deadlock_queue(
                type, {}, defaultValuesConfig["DeadLock_Queue"]
            )
            output_ini["ProductType"] = self.transform_product_type(
                type, production_ini["ProductType"]
            )
            output_ini["Transporter"] = self.transform_transporter(
                type,
                production_ini["Transporter"],
                defaultValuesConfig,
                production_ini["ChangeTime"],
            )
            ...
            output_ini["Machine"] = self.transform_machine(
                type, production_ini["Machine"], defaultValuesConfig
            )
            output_ini["Start_Queue"] = self.transform_start_queue(
                type, production_ini["Start_Queue"]
            )
            output_ini["End_Queue"] = self.transform_end_queue(
                type, production_ini["End_Queue"]
            )
            output_ini["DeadLock_Queue"] = self.transform_deadlock_queue(
                type,
                production_ini["DeadLock_Queue"],
            ...
            output_ini["Transporter"] = self.transform_transporter(
                type, production_ini["Transporter"], defaultValuesConfig
            )
        ...
        output_ini["Defect"] = self.transform_defect(
            type, production_ini["Defect"], defaultValuesConfig["Distribution"]
        )

        output_ini["Repair"] = self.transform_repair(type, production_ini["Repair"])
        output_ini["RandomSeed"] = self.transform_random_seed(
            type, production_ini["RandomSeed"]
        )
        return output_ini

    def transform_machine(
        self, type="lvl2", old_machine_ini_dict={}, defaultValues={}, changeTime={}
    ):
        """
        ...
        if type == "lvl2":
            ...
            for old_machine_dict in old_machine_dict_list:
                ...
                if "waiting_time" not in old_machine_dict.keys():
                    machine_dict["waiting_time"] = defaultValues["Machine"][
                        "waiting_time"
                    ]
                else:
                ...
                queue_dict["number_of_positions"] = old_machine_dict[
                    "number_of_positions"
                ]
            ...
            return {
                "machine_dict": machine_dict_list,
                "queue_process_dict": queue_process_dict_list,
            ...
            for old_machine_dict in old_machine_dict_list:
                            ...
                            set_up_list.append(
                                {
                                    "start": process_1,
                                    "end": process_2,
                                    "mean": changeTime["set_up_time"],
                                    "deviation": changeTime["deviation"],
                ...
                queue_dict["add_time"] = {
                    "mean": changeTime["add_time"],
                    "deviation": changeTime["deviation"],
                    "type": defaultValues["Distribution"]["type"],
                }
                queue_dict["remove_time"] = {
                    "mean": changeTime["remove_time"],
                    "deviation": changeTime["deviation"],
                    "type": defaultValues["Distribution"]["type"],
                }
                ...
                queue_process_dict["number_of_positions"] = 1
                queue_process_dict["location"] = old_machine_dict["location"]
                queue_process_dict["add_time"] = {
                    "mean": changeTime["add_time"],
                    "deviation": changeTime["deviation"],
                    "type": defaultValues["Distribution"]["type"],
                }
                queue_process_dict["remove_time"] = {
                    "mean": changeTime["remove_time"],
                    "deviation": changeTime["deviation"],
                    "type": defaultValues["Distribution"]["type"],
                }
            ...
            self.checkWaitingTimeMachine(
                machineList=old_machine_ini_dict["machine_dict"]
            )
    ...
    def transform_start_queue(
        self, type="lvl2", old_start_queue_ini_dict={}, default_value_config={}
    ):
        """
    ...
    def transform_end_queue(
        self, type="lvl2", old_end_queue_ini_dict={}, default_value_config={}
    ):
        """
    ...
    def transform_deadlock_queue(
        self, type="lvl2", old_deadlock_queue_ini_dict={}, default_value_config={}
    ):
        """
        ...
        if type == "lvl2":
            if len(old_deadlock_queue_ini_dict.keys()) == 0:
                ...
                if "deadlock_waiting_time" not in old_deadlock_queue_ini_dict.keys():
                    ...
                    new_deadlock_queue_ini_dict["deadlock_waiting_time"] = (
                        default_value_config["deadlock_waiting_time"]
                    )
                else:
                    new_deadlock_queue_ini_dict = old_deadlock_queue_ini_dict
    ...
    def transform_product_type(self, type="lvl2", old_product_type_ini_dict={}):
        ...
        if type == "lvl2":
            ...
            new_product_type_ini_dict["settings"] = old_product_type_ini_dict[
                "settings"
            ]

            return new_product_type_ini_dict

    def transform_transporter(
        self, type="lvl2", old_transporter_ini_dict={}, defaultValues={}, changeTime={}
    ):
        """
        ...
        if type == "lvl2":
            ...
            for setting_dict in new_transporter_ini_dict["settings"]:
                if "waiting_time" not in setting_dict.keys():
                    setting_dict["waiting_time"] = defaultValues["Transporter"][
                        "waiting_time"
                    ]
            ...
            new_transporter_ini_dict["start_location"] = defaultValues["Transporter"][
                "start_location"
            ]

            setting_dict = {
                "number_of_positions": old_transporter_ini_dict["settings"][
                    "number_of_positions"
                ],
                "location_id": defaultValues["Transporter"]["start_location"][0]["id"],
                "route": {"type": "free"},
                "speed": old_transporter_ini_dict["settings"]["speed"],
    ...
    def transform_defect(
        self, type="lvl2", old_defect_ini_dict={}, default_value_dict={}
    ):
        """
        ...
        if type == "lvl2":
            ...
            new_defect_ini_dict["transporter_normal"] = [
                old_defect_ini_dict["transporter_normal"]
            ]
            new_defect_ini_dict["transporter_random"] = default_value_dict[
                "defect_random"
            ]

            new_defect_ini_dict["machine_normal"] = [
                old_defect_ini_dict["machine_normal"]
            ]
