...
def create_petrinet(
    product_type_id: int, product_paths: list, pnml_save_path: str, visualize=False
):
        ...
        for node in product_path:
            if node[:2] == "t_":
                ...
                transition = PetriNet.Transition(
                    node, str(product_paths_petri.index(product_path))
                )  # add product index as label
            ...
            if not node[2:] == "sink":
                ...
                if node_1[:2] == "t_":
                    n_1 = find_in_set(
                        net.transitions,
                        node_1,
                        label=str(product_paths_petri.index(product_path)),
                    )  # also read the index to not doubleuse transitions
    ...
    pnml_exporter.apply(
        net, Marking(), pnml_save_path + "/product_" + str(product_type_id) + ".pnml"
    )
