...
class ServiceControllerTransporter(ServiceController):
    ...
    def evaluateService(self, event_onto):
        ...
        if service_onto.free_service_operator > 0:
            ...
            service_operator[0].has_for_transporter_service_operator_transporter.append(
                transport_onto
            )
            ...
            sub_defect = [
                sub_defect
                for sub_defect in transport_onto.has_for_defect_transporter[
                    0
                ].has_for_sub_defect
                if sub_defect.defect_type == transport_onto.defect_type_transporter
            ][0]
            repair_distribution = sub_defect.has_for_repair_distribution[0]

            time = time + self.repair_service.simCore.distribution.getTimefromOnto(
                repair_distribution
            )

            event_onto = self.repair_service.simCore.event.createEvent(
                time,
                Transporter_Enum.Defect,
                time - transport_onto.next_defect_transporter,
            )
            self.repair_service.simCore.event.add_transport_to_event(
                event_onto, transport_onto
            )
            self.repair_service.simCore.event.add_service_to_event(
                event_onto,
...
class ServiceControllerMachine(ServiceController):
    ...
    def evaluateService(self, event_onto):
        ...
        if service_onto.free_service_operator > 0:
            ...
            service_operator[0].has_for_machine_service_operator_machine.append(
                machine_onto
            )
            ...
            sub_defect = [
                sub_defect
                for sub_defect in machine_onto.has_for_defect_machine[
                    0
                ].has_for_sub_defect
                if sub_defect.defect_type == machine_onto.defect_type_machine
            ][0]
            repair_distribution = sub_defect.has_for_repair_distribution[0]
            ...
            event_onto = self.repair_service.simCore.event.createEvent(
                time, Machine_Enum.Defect, time - machine_onto.next_defect_machine
            )
            self.repair_service.simCore.event.add_machine_to_event(
                event_onto, machine_onto
            )
            self.repair_service.simCore.event.add_service_to_event(
                event_onto,
