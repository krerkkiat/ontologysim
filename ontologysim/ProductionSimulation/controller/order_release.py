...
class OrderReleaseController:
    ...
    def evaluateCreateOrderRelease(self):
        ...
        if self.orderRelease.max_number > self.orderRelease.current_number_of_products:
            ...
            event_list = self.orderRelease.simCore.onto[
            ...
            for event in event_list:
                if (
                    event.type == OrderRelease_Enum.Release.value
                    or event.type == Evaluate_Enum.OrderRelease.value
                ):
                    number_of_events = 1
                    break
            ...
            for deadlock_queue in self.orderRelease.simCore.central.dead_lock_list:
                number_of_free_positions_deadlock = (
                    self.orderRelease.simCore.queue.get_number_of_free_positions(
                        deadlock_queue
                    )
                )
            ...
            if (
                ...
                and number_of_todos > 0
                and number_of_free_positions_deadlock > 0
            ):
                ...
                event_onto = self.orderRelease.simCore.event.createEvent(
                    self.orderRelease.simCore.getCurrentTimestep(),
                    Evaluate_Enum.OrderRelease,
                    0,
                )
    ...
    def evaluateOrderRelease(self, event_onto):
        ...
        if number_free_position > 0:
            ...
            if len(task_list) > 0:
                ...
                self.orderRelease.create_release(
        ...
        if self.orderRelease.simCore.central.number_of_start_positions < 2:
            event_onto = self.orderRelease.simCore.event.createEvent(
                self.orderRelease.simCore.getCurrentTimestep() + 5,
                Evaluate_Enum.OrderRelease,
                0,
            )
    ...
    def getTaskList(self, event_onto):
        ...
        if not self.orderRelease.simCore.central.simInstance.allStartTasksFinished:
            task_list = [
                [task, task.todo_number, task.has_for_product_type_task.__getitem__(0)]
                for task in self.orderRelease.simCore.central.task_list
                if task.todo_number > 0 and task.task_type == "start"
            ]

            if len(task_list) == 0:
                self.orderRelease.simCore.central.simInstance.allStartTasksFinished = (
                    True
                )
            else:
                createNewEvaluationEvent = True
        if (
            self.orderRelease.simCore.central.simInstance.allStartTasksFinished
            and not self.orderRelease.simCore.central.simInstance.allLoggingTasksFinished
        ):
            for task in self.orderRelease.simCore.central.task_list:
                    ...
                    if task.start_time > time and not createNewEvaluationEvent:
                        ...
                        task_list.append(
                            [
                                task,
                                task.todo_number,
                                task.has_for_product_type_task.__getitem__(0),
                            ]
                        )
        ...
        if (
            self.orderRelease.simCore.central.simInstance.allStartTasksFinished
            and self.orderRelease.simCore.central.simInstance.allLoggingTasksFinished
        ):
            task_list = [
                [task, task.todo_number, task.has_for_product_type_task.__getitem__(0)]
                for task in self.orderRelease.simCore.central.task_list
                if task.todo_number > 0 and task.task_type == "end"
            ]
