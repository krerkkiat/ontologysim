...
class TransporterController_NJF(TransporterController.TransporterController):
    ...
    def sort_products_on_transporter(self, transport_onto):
        ...
        """
        event_list = []
        location_transport_onto = transport_onto.has_for_transp_queue[
            0
        ].has_for_queue_location[0]
            ...
            for position in queue.has_for_position:
                for product in position.has_for_product:
                    if (
                        ...
                        process_id_list = self.transport.simCore.product.getNextProcess(
                            product
                        )
                        erg_list = []

                        if len(process_id_list) == 0:
                            queue_onto = self.transport.simCore.central.end_queue_list[
                                0
                            ]

                            distance = self.transport.simCore.location.distance_dict[
                                queue_onto.has_for_queue_location[0].name
                            ][location_transport_onto.name]
                            for event in event_onto_list:
                                if event.type == Queue_Enum.Change.value:
                                    event_list.append(
                                        [product.name, event.time, distance, queue_onto]
                                    )
                            ...
                            for process_id in process_id_list:
                                ...
                                suitableMachine = self.transport.getAlowedMachine(
                                    transport_onto, suitableMachine
                                )
                            ...
                            if len(erg_list) > 0:
                                ...
                                for erg in erg_list:
                                    ...
                                    for queue_onto in machine_onto.has_for_input_queue:
                                        positions = self.transport.simCore.queue.get_free_positions(
                                            queue_onto
                                        )
                                        ...
                                        if len(positions) > 0:
                                            # for event in event_onto_list:
                                            distance = self.transport.simCore.location.distance_dict[
                                                queue_onto.has_for_queue_location[
                                                    0
                                                ].name
                                            ][location_transport_onto.name]
        ...
        for k, v, w, z in event_list:
            res[k].append([v, w, z])

        products_on_transporter = [
            [k, v[-1][0], v[-1][1], v[-1][2]] for k, v in res.items()
        ]
                    ...
                    for product in position.has_for_product:
                        if (
                            ...
                            for event in event_onto_list:
                                if event.type == Queue_Enum.Change.value:
                                    distance = (
                                        self.transport.simCore.location.distance_dict[
                                            location_transport_onto.name
                                        ][queue_location.name]
                                    )
                                    event_list.append(
                                        [product.name, event.time, distance, queue]
                                    )
    ...
    def evaluateTransport(self, event_onto):
        ...
        for queue in queue_list:
            freePlace += self.transport.simCore.queue.get_number_of_free_positions(
                queue
            )
        ...
        for element in elements:
            ...
            if type == type_on:
                if queue_onto != None:
                    if (
                        ...
                        time = self.transport.createTransportation(
                            transport_onto, queue_onto, time
                        )
                        ...
                        event_onto = self.transport.simCore.event.createEvent(
                            time, Evaluate_Enum.ProductFinished, 0
                        )
                        self.transport.simCore.event.add_product_to_event(
                            event_onto, product_onto
                        )
                ...
                if not (
                    self.transport.compare_location_product_transporter(
                        product_onto, transport_onto
                    )
                ):
                    time = self.transport.createTransportation(
                        transport_onto,
                        product_onto.is_position_of[0]
                        .is_queue_of[0]
                        .has_for_queue_location[0],
                        time,
                    )
                ...
                time = self.transport.simCore.queue.create_change(
                    product_onto, new_position_onto, time, Queue_Enum.AddToTransporter
                )
        ...
        if not createEvaluation and len(elements) > 0:
            ...
            for start_queue in self.transport.simCore.central.start_queue_list:
                ...
                if len(positions) > 0:
                    time = self.transport.createTransportation(
                        transport_onto, start_queue, time
                    )
                    position_onto = positions[0]
                ...
                for deadlock_queue in self.transport.simCore.central.dead_lock_list:
                    ...
                    if len(positions) > 0:
                        distance = self.transport.simCore.location.calculateDistance(
                            transport_onto.has_for_transp_queue[
                                0
                            ].has_for_queue_location[0],
                            deadlock_queue.has_for_queue_location[0],
                        )
                        decide_deadlock_queue.append(
                            [positions, distance, len(positions), deadlock_queue]
                        )

                # sort after distance
                decide_deadlock_queue = sorted(
                    decide_deadlock_queue, key=operator.itemgetter(1, 2)
                )

                if len(decide_deadlock_queue) > 0:
                    # print(decide_deadlock_queue[0])
                    time = self.transport.createTransportation(
                        transport_onto, decide_deadlock_queue[0][3], time
                    )
            ...
            if position_onto != None and product_onto != None:
                ...
                time = self.transport.createWait(
                    transport_onto.transporter_waiting_time, time, transport_onto
                )
            createEvaluation = True
            ...
            number_of_events = [
                event
                for event in transport_onto.is_transport_event_of
                if self.transport.simCore.onto[Label.EventList.value + "0"]
                in event.is_event_list_of
            ]
        ...
        if createEvaluation:
            event_onto = self.transport.simCore.event.createEvent(
                time, Evaluate_Enum.Transporter, 0
            )
            self.transport.simCore.event.add_transport_to_event(
                event_onto, transport_onto
            )
    ...
    def combine_products(self, transport_onto):
        ...
        products_not_on_transporter = self.sort_products_not_on_transporter(
            transport_onto
        )

        # combine products on transporter and not on transporter
        elements = self.sort_products(
            products_on_transporter, products_not_on_transporter
        )
