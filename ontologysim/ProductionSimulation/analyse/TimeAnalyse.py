...
class TimeAnalyse:
    ...
    def time_analyse(info_string, is_start):
        ...
        if is_start:
            ...
            TimeAnalyse.time_current_dict[info_string] = (
                datetime.datetime.now() - TimeAnalyse.time_current_dict[info_string]
            )
            if info_string in TimeAnalyse.time_dict.keys():
                TimeAnalyse.time_dict[info_string] += TimeAnalyse.time_current_dict[
                    info_string
                ]
            else:
                TimeAnalyse.time_dict[info_string] = TimeAnalyse.time_current_dict[
