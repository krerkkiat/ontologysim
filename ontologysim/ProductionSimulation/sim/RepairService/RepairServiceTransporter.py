...
class RepairServiceTransporter(RepairService):
    ...
    def createService(self, number_of_repair):
        ...
        if number_of_repair > 0:
            ...
            for i in range(number_of_repair):
                service_onto.has_for_transporter_service_operator.append(
    ...
    def createServiceOperator(self):
        ...
        service_operator = self.simCore.central.transporter_service_operator_class(
            Label.TransporterServiceOperator.value
            + str(self.simCore.transporter_service_operator_id)
        )
    ...
    def getFreeServiceOperator(self, service_onto):
        ...
        return [
            service_operator
            for service_operator in service_onto.has_for_transporter_service_operator
            if len(service_operator.has_for_transporter_service_operator_transporter)
            == 0
        ]
    ...
    def repair(self, event_onto):
        ...
        self.simCore.transport.setNextDefectTime(
            transport_onto, transport_onto.has_for_defect_transporter[0]
        )
    ...
    def evaluateCreateEvent(self, service_onto, transport_onto):
        ...
        for event in event_list:
            if (
                event.type
                in [transport_enum.value for transport_enum in Transporter_Enum]
                or event.type == Evaluate_Enum.Transporter.value
            ):
        ...
        if (
            number_of_events_service_onto == 0
            and len(service_onto.has_for_wait_transporter_service) > 0
            and service_onto.free_service_operator > 0
        ):
            event_onto = self.simCore.event.createEvent(
                self.simCore.getCurrentTimestep(), Evaluate_Enum.TransporterDefect, 0
            )
