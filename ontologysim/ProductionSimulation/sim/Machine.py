...
class Machine:
    ...
    def createMachine(self, machine_dict, process_config_list):
        ...
        machineInstance = self.simCore.central.machine_class(
            Label.Machine.value + str(machine_dict["machine_id"]),
        ...
        self.addQueue(
            machineInstance,
            machine_dict["queue_input_id"],
            machine_dict["queue_output_id"],
        )
    ...
    def addProcess(self, machineInstance, process_id_list):
        ...
        for prod_process_dict in process_id_list:
            prod_processInstance = self.simCore.prod_process.createProdProces(
                prod_process_dict, machineInstance
            )
            machineInstance.has_for_prodprocess.append(prod_processInstance)

        machineInstance.has_for_last_process.append(
            machineInstance.has_for_prodprocess[0]
        )
    ...
    def addSetup(self, machineInstance, set_up_dict):
        ...
        for set_up in set_up_dict:
            setUpInstance = self.simCore.central.setup_class(
                Label.SetUp.value + str(self.simCore.set_up_id)
            )
    ...
    def addQueue(self, machineInstance, queue_input_id_list, queue_output_id_list):
        ...
        queue_standard_id_list = list(
            set(queue_input_id_list).intersection(set(queue_output_id_list))
        )
        queue_input_id_list = list(
            set(queue_input_id_list).difference(set(queue_standard_id_list))
        )
        queue_output_id_list = list(
            set(queue_output_id_list).difference(set(queue_standard_id_list))
        )
        ...
        for queue_standard_id in queue_standard_id_list:
            ...
            queueInstance.is_a = [
                ...
                self.simCore.central.outputqueue_class,
            ]
            machineInstance.has_for_output_queue.append(queueInstance)
            machineInstance.has_for_input_queue.append(queueInstance)
    ...
    def transform_process_id_into_process(self, process_id, machine_onto):
        ...
        prod_process = [
            ...
            if int(self.simCore.prod_process.getID(prod_process)) == int(process_id)
        ]
    ...
    def setNextDefectTime(self, machine_onto, defect_onto):
        ...
        :param defect_onto: onto
        """
        next_defect_time, machine_defect_type = self.simCore.defect.getNextDefectTime(
            defect_onto
        )
    ...
    def evaluateDefect(self, event_onto):
        ...
        if self.simCore.defect.machine_defect_possible:
            ...
            if machine_onto.next_defect_machine <= time:
                ...
                event_onto = self.simCore.event.createEvent(
                    time, Evaluate_Enum.MachineDefect, 0
                )
    ...
    def create_set_up(self, process_onto, time):
        ...
        set_up_list = [
            set_up
            for set_up in process_onto.is_end_process_of
            if last_process_onto in set_up.has_for_start_process
        ]
        ...
        if distribution is not None:
            ...
            time = time + time_diff
            event_onto = self.simCore.event.createEvent(
                time, Machine_Enum.SetUp, time_diff
            )
    ...
    def create_process(self, product_onto_list, process_onto, time):
        ...
        time = time + time_diff
        event_onto = self.simCore.event.createEvent(
            time, Machine_Enum.Process, time_diff
        )
    ...
    def wait(self, event_onto):
        ...
        """
        machine_onto = event_onto.has_for_machine_event.__getitem__(0)
        input_queue_current_size = sum(
            [queue.current_size for queue in machine_onto.has_for_input_queue]
        )
        output_queue_current_size = sum(
            [
                self.simCore.queue.get_number_of_free_positions(queue) / queue.size
    ...
    def createWait(self, waiting_time, time, machine_onto):
        ...
        event_onto = self.simCore.event.createEvent(
            time_diff + time, Machine_Enum.Wait, time_diff
        )
        self.simCore.event.add_machine_to_event(event_onto, machine_onto)
                ...
                [
                    {
                        ...
                        "product_name_list": [
                            product_onto.name for product_onto in product_onto_list
                        ],
    ...
    def transformToDict(self, id):
        ...
        if id == "all":
            machine_list = [
                machine_onto
                for machine_onto in self.simCore.onto.search(
        ...
        for machine_onto in machine_list:
            ...
            response_dict[machine_onto.name]["input_queue"] = [
                queue_onto.name for queue_onto in machine_onto.has_for_input_queue
            ]
            response_dict[machine_onto.name]["output_queue"] = [
                queue_onto.name for queue_onto in machine_onto.has_for_output_queue
            ]
            response_dict[machine_onto.name]["process_queue"] = [
                queue_onto.name for queue_onto in machine_onto.has_for_queue_process
            ]
            response_dict[machine_onto.name]["location"] = (
                self.simCore.location.transformToDict(
                    machine_onto.has_for_machine_location[0].name
                )
            )
            response_dict[machine_onto.name]["prod_process"] = {}
            for prod_process in machine_onto.has_for_prodprocess:
                distribution_dict = self.simCore.distribution.transformToDict(
                    prod_process.has_for_prod_distribution[0].name
                )
                del distribution_dict["label"]

                response_dict[machine_onto.name]["prod_process"][prod_process.name] = {
                    "label": prod_process.name,
                    **distribution_dict,
                }
