...
class Transporter:
    ...
    def createTransporter(self, setting_dict, location_onto):
        ...
        queueInstance = self.simCore.queue.createQueue(
            setting_dict, Label.Queue.value, self.simCore.queue_id
        )
        ...
        event_onto = self.simCore.event.createEvent(
            self.simCore.getCurrentTimestep(), Evaluate_Enum.Transporter, 0
        )
    ...
    def getAlowedMachine(self, transport_onto, suitable_list):
        ...
        if self.allowed_queues[transport_onto.name]["type"] == "free":
            ...
            for setting_list in suitable_list:
                for allowed_machine in self.allowed_queues[transport_onto.name][
                    "machine_input_list"
                ]:
        ...
        for k, v in self.allowed_queues.items():
            if v["type"] != "free":
                ...
                for queue_id in v["queue_list"]:
                    ...
                    for (
                        queue
                    ) in self.simCore.central.queue_list_not_transport_end_process:
    ...
    def createWait(self, waiting_time, time, transport_onto):
        ...
        """
        event_onto = self.simCore.event.createEvent(
            waiting_time + time, Transporter_Enum.Wait, waiting_time
        )
    ...
    def transport(self, event_onto):
        ...
        old_location = transport_onto.has_for_transp_queue.__getitem__(
            0
        ).has_for_queue_location.__getitem__(0)
        transport_onto.has_for_transp_queue.__getitem__(
            0
        ).has_for_queue_location.clear()
        transport_onto.has_for_transp_queue.__getitem__(
            0
        ).has_for_queue_location.append(new_location_onto)
        current_location_time = (
            event_onto.time
            - transport_onto.start_current_location
            - event_onto.time_diff
        )
    ...
    def createTransportation(self, transport_onto, new_location_onto, time):
        ...
        if ".Location" in str(type(new_location_onto)):
            ...
            new_location_onto = new_location_onto.has_for_queue_location.__getitem__(0)
        elif ".InputQueue" in str(new_location_onto.is_a) or ".OutputQueue" in str(
            new_location_onto.is_a
        ):
            new_location_onto = new_location_onto.has_for_queue_location.__getitem__(0)
            ...
            raise Exception(
                "change location object not defined " + str(new_location_onto.is_a)
            )

        distance = self.simCore.location.calculateDistance(
            old_location, new_location_onto
        )
        ...
        event_onto = self.simCore.event.createEvent(
            time, Transporter_Enum.Transport, travel_time
        )
    ...
    def compare_location_product_transporter(self, product_onto, transport_onto):
        ...
        """
        location_product = (
            product_onto.is_position_of.__getitem__(0)
            .is_queue_of.__getitem__(0)
            .has_for_queue_location.__getitem__(0)
        )
        location_transport = transport_onto.has_for_transp_queue.__getitem__(
            0
        ).has_for_queue_location.__getitem__(0)

        return (
            location_product.x == location_transport.x
            and location_product.y == location_transport.y
            and location_product.z == location_transport.z
        )
    ...
    def evaluateDefect(self, event_onto):
        ...
        if self.simCore.defect.transport_defect_possible:
            ...
            if transport_onto.next_defect_transporter <= time:
                ...
                self.simCore.repair_service_transporter.addDefectToService(
                    transport_onto
                )
                self.simCore.event.store_event(event_onto)
                self.simCore.event.remove_from_event_list(event_onto)
                event_onto = self.simCore.event.createEvent(
                    time, Evaluate_Enum.TransporterDefect, 0
                )
                self.simCore.event.add_service_to_event(
                    event_onto, self.simCore.repair_service_transporter.service_onto
                )
    ...
    def setNextDefectTime(self, transporter_onto, defect_onto):
        ...
        :param defect_onto:
        """
        next_defect_time, defect_type = self.simCore.defect.getNextDefectTime(
            defect_onto
        )
    ...
    def transformToDict(self, id):
        ...
        if id == "all":
            transport_list = [
                transport_onto
                for transport_onto in self.simCore.onto.search(
        ...
        for transport_onto in transport_list:
            ...
            response_dict[transport_onto.name]["queue"] = (
                transport_onto.has_for_transp_queue[0].name
            )
            response_dict[transport_onto.name]["location"] = (
                self.simCore.location.transformToDict(
                    transport_onto.has_for_transp_queue[0]
