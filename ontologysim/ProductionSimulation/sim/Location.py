...
class Location:
    ...
    def createLocation(self, location_list):
        ...
        :return: onto
        """
        locationInstance = self.simCore.central.location_class(
            Label.Location.value + str(self.simCore.location_id)
        )
    ...
    def init_distance_dict(self):
        ...
        """
        location_list = self.simCore.onto.search(
            type=self.simCore.central.location_class
        )
        ...
        for i1 in range(a):
            for i2 in range(b):
                if i1 == i2:
                    self.distance_dict[location_list[i1].name][
                        location_list[i2].name
                    ] = 0
                else:
                    distance = self.calculateDistance(
                        location_list[i1], location_list[i2]
                    )
                    self.distance_dict[location_list[i1].name][
                        location_list[i2].name
                    ] = distance
                    self.distance_dict[location_list[i2].name][
                        location_list[i1].name
                    ] = distance
            b = b - 1
    ...
    def transformToDict(self, id):
        ...
        response_dict["queue"] = self.location_queue_dict["location"][
            location_onto.name
        ]
