...
class Queue:
    ...
    def createQueue(self, setting_dict, label, id=-1):
        ...
        distribution_add_onto = self.simCore.distribution.createDistribution(
            distribution_add_dict, "add_queue" + str(id)
        )
        distribution_remove_onto = self.simCore.distribution.createDistribution(
            distribution_remove_dict, "remove_queue" + str(id)
        )
    ...
    def change(self, event_onto):
        ...
        if event_onto.additional_type == Queue_Enum.RemoveFromTransporter.value:
            ...
            event_onto.additional_type == Queue_Enum.RemoveFromTransporterDeadlock.value
        ):
            product_onto.blocked_for_transporter = 1

            block_event_onto = self.simCore.event.createEvent(
                event_onto.time + self.deadlockWaitingTime,
                Product_Enum.EndBlockForTransporter,
                self.deadlockWaitingTime,
            )
        ...
        self.simCore.logger.evaluatedInformations(
            [
                {
                    "type": event_onto.additional_type,
        ...
        if not (
            event_onto.additional_type == Queue_Enum.RemoveFromTransporter.value
            and product_onto.has_for_product_state[0].state_name == "sink"
        ):
    ...
    def create_change(
        self, part_onto, new_position_onto, time, event_type=Queue_Enum.Default
    ):
        """
        ...
        remove_distribution = (
            (part_onto.is_position_of.__getitem__(0)).is_queue_of.__getitem__(0)
        ).has_for_remove_time.__getitem__(0)
    ...
    def create_change_for_start_queue(
        self, part_onto, new_position_onto, time, transportation_onto=None
    ):
        """
    ...
    def get_free_positions(self, queue_onto):
        ...
        positions = [
            position for position in position_list if position.blockedSpace == 0
        ]
    ...
    def evaluateAddRemove(self, new_position_onto, oldPosition, event_onto):
        ...
        for machine_onto in machine_list:
            event_list = [
                event
                for event in machine_onto.is_machine_event_of
                if self.simCore.onto[Label.EventList.value + "0"]
            ...
            if len(event_list) == 2:
                ...
                wait_event_onto.time_diff = wait_event_onto.waiting_time + (
                    time - old_event_time
                )
                ...
                output_queue_current_size = sum(
                    [
                        self.simCore.queue.get_number_of_free_positions(queue)
                        / queue.size
    ...
    def transformToDict(self, id):
        ...
        if id == "all":
            id_list = [
                ...
                for queue_onto in self.simCore.onto.search(
                    type=self.simCore.central.queue_class
                )
            ]
        else:
            id_list = [id]
        ...
        for id in id_list:
            ...
            response_dict[queue_onto.name]["positions"] = [
                position.name for position in queue_onto.has_for_position
            ]
