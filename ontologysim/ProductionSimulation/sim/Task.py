...
class Task:
    ...
    def createTask(
        self, product_type_onto, number_of_products, task_type="logging", start_time=0
    ):
        """
        ...
        :param task_type: str
        """
        taskInstance = self.simCore.central.task_class(
            Label.Task.value + str(self.simCore.task_id)
        )
    ...
    def updateTaskList(self):
        ...
        """
        self.simCore.central.task_list = self.simCore.onto.search(
            type=self.simCore.central.task_class
        )
    ...
    def transformToDict(self, id):
        ...
        if id == "all":
            task_list = [
                task_onto
                for task_onto in self.simCore.onto.search(
                    type=self.simCore.central.task_class
                )
            ]
        else:
        ...
        for task_onto in task_list:
            ...
            response_dict[task_onto.name]["product_type"] = (
                task_onto.has_for_product_type_task[0].name
            )
