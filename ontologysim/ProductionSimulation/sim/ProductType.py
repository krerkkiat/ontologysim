...
class ProductType:
    ...
    def init_percentage_dict(self):
        ...
        """
        # TODO merge
        product_type_list = self.simCore.onto.search(
            type=self.simCore.central.product_type_class
        )
        ...
        for productTypeOnto in product_type_list:
            ...
            for stateOnto in stateOntoList:
                self.percentageDict[productTypeOnto.name][stateOnto.state_name] = (
                    self.iterateAverageProductTree(stateOnto)
                )
            ...
            for stateOnto in stateOntoList:
                if stateOnto.state_name != "source":
                    self.percentageDict[productTypeOnto.name][stateOnto.state_name] = (
                        ...
                        / self.percentageDict[productTypeOnto.name]["source"]
                    )
    ...
    def iterateAverageProductTree(self, state):
        ...
        return (
            min([self.iterateAverageProductTree(state) for state in nextStateList]) + 1
        )
    ...
    def createProductType(self, product_type_config, process_config):
        ...
        :return:
        """
        product_typeInstance = self.simCore.central.product_type_class(
            Label.ProductType.value + str(self.simCore.product_type_id)
        )
        ...
        for process_list in process_product_type_config:
            if len(process_list) == 1:
                ...
                process_onto = self.simCore.onto[
                    Label.Process.value + str(process_list[0])
                ]
                processInstance.is_for_prod_type_process_of.append(process_onto)
                sourceStateInstance.has_for_prod_type_process_state.append(
                    processInstance
                )
                ...
                if "merged" in process_config.keys():
                    for element in process_config["merged"]["in"]:
                        ...
                        if process_onto.process_id == process_list[0]:
                            for (
                                combine_process_data_onto
                            ) in process_onto.has_for_input_combine:
                                ...
                                if (
                                    int(
                                        product_typeInstance.name.replace(
                                            Label.ProductType.value, ""
                                        )
                                    )
                                    == int(element["product_type"])
                                    and len(
                                        combine_process_data_onto.has_for_state_combine_process_data
                                    )
                                    == 0
                                ):
                                    # combine_process_data_onto.number_state = int(element["number"])
                                    combine_process_data_onto.has_for_state_combine_process_data.append(
                                        sourceStateInstance
                                    )
                                    break
                    ...
                    for element in process_config["merged"]["out"]:
                        if process_onto.process_id == process_list[0]:
                            ...
                            for (
                                combine_process_data_onto
                            ) in process_onto.has_for_output_combine:
                                if (
                                    int(
                                        product_typeInstance.name.replace(
                                            Label.ProductType.value, ""
                ...
                for process in process_list[1:-1]:
                    processInstance = self.simCore.central.prod_type_process_class(
                        Label.ProdTypeProcess.value
                        + str(self.simCore.prod_type_process_id)
                    )
                    processInstance.is_for_prod_type_process_of.append(
                    ...
                    stateInstance.has_for_prod_type_process_state.append(
                        processInstance
                    )
    ...
    def getProdProcessId(self):
        ...
        process_id_list = []
        for prod_type in self.simCore.onto.search(
            type=self.simCore.central.product_type_class
        ):
    ...
    def transformToDict(self, id):
        ...
        if id == "all":
            product_type_list = [
                product_type_onto
                for product_type_onto in self.simCore.onto.search(
                    type=self.simCore.central.product_type_class
                )
            ]
        else:
        ...
        for product_type_onto in product_type_list:
            ...
            response_dict[product_type_onto.name]["state"] = [
                state_onto.name
                for state_onto in product_type_onto.is_product_type_of_state
            ]
            response_dict[product_type_onto.name]["source_state"] = [
                state_onto.name
                for state_onto in product_type_onto.is_product_type_of_source
            ]
    ...
    def getCompleteProductTypePath(self, label):
        ...
        state_list = [
            state_onto for state_onto in product_type_onto.is_product_type_of_state
        ]
        ...
        for state_onto in state_list:
            response_dict["path"].append(
                {
                    "state": state_onto.name,
                    "forward_process": [
                        prod_type_process_onto.name
                        for prod_type_process_onto in state_onto.has_for_prod_type_process_state
                    ],
                    "reverse_process": [
                        prod_type_process_onto.name
                        for prod_type_process_onto in state_onto.has_for_reverse_prod_type_process_state
                    ],
                }
            )
        ...
        for product_type_process_onto in product_type_process_list:
            ...
            response_dict["process_info"].append(
                {
                    "product_type_process_name": product_type_process_onto.name,
                    "name": process_onto.name,
                    "state_onto": product_type_process_onto.is_prod_type_process_of_state[
                        0
                    ].name,
                    "process_id": process_onto.process_id,
                }
            )
        ...
        while not iterationFinished:
            for state_onto in stateList:
                ...
                for process_onto in process_list:
                    ...
                    stateNumberDict[state_onto2.name] = (
                        stateNumberDict[state_onto.name] + 1
                    )
            ...
            if len(stateListNextIteration) == 0:
                ...
                stateList = [
                    state_onto
                    for state_onto in stateListNextIteration
                    if state_onto.has_for_prod_type_process_state != []
                ]
                stateListNextIteration = []

            i += 1

        response_dict["max_state"] = stateNumberDict[
            max(stateNumberDict, key=stateNumberDict.get)
        ]
