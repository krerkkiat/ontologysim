...
class Event:
    ...
    def createEvent(self, time, type, time_diff):
        ...
        if len(self.event_storage) == 0:
            ...
            if eventInstance == None:
                ...
                try:
                    self.simCore.onto[eventName].name = Label.Event.value + str(
        ...
        self.simCore.onto[Label.EventList.value + "0"].has_for_event.append(
            eventInstance
        )
    ...
    def getNumberOfEvents(self):
        ...
        number_of_events = len(
            [
                event.time
                for event in event_list
                if event.time >= self.simCore.getCurrentTimestep()
            ]
        )
    ...
    def add_to_short_term_logger(self, event_onto, position_onto):
        ...
        ].has_for_event_short_term_logger.append(event_onto)
        ...
        for event_onto in event_list:
            if (
                self.simCore.onto[Label.ShortTermLogger.value + "0"]
                in event_onto.is_event_short_term_logger_of
            ):
                number_of_events += 1
        ...
        if number_of_events >= 2:
            ...
            self.simCore.onto[
                Label.ShortTermLogger.value + "0"
            ].has_for_event_short_term_logger.remove(event_onto)
    ...
    def add_to_logger(self, event_onto):
        ...
        """
        if (
            "csv" in self.simCore.event_logger.type
            or "database" in self.simCore.event_logger.type
        ):
            ...
            if self.first_run == True:
                ...
                ].has_for_event_of_logger.append(eventInstance)
            else:
                eventInstance = self.simCore.onto[
                    Label.Logger.value + "1"
                ].has_for_event_of_logger[self.number_logger]
            ...
            if self.first_run == False:
                if old_event_instance_type in self.property_dict_logger.keys():
                    ...
                    for prop in properties:
                        prop_name = prop.name
                        self.property_dict_logger[old_event_instance_type].append(
                            prop_name
                        )
                        self.removeProperties_EventLogger(prop_name, eventInstance)
            ...
            if event_onto.type in self.property_dict.keys():
                for prop_name in self.property_dict[event_onto.type]:
                    self.swapProperties_EventLogger(
                        prop_name, eventInstance, event_onto
                    )
            else:
                properties = event_onto.get_properties()
                self.property_dict[event_onto.type] = []
            ...
            if self.number_logger > self.number_of_events_in_logger:
                ...
                if (
                    "database" in self.simCore.event_logger.type
                    and self.simCore.event_logger.is_activated
                ):
