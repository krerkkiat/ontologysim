...
class Defect:
    ...
    def createDefect(self, random_dict, normal_distribution_list):
        ...
        :return: defect_onto
        """
        defect_onto = self.simCore.central.defect_class(
            Label.Defect.value + str(self.simCore.defect_id)
        )
        random_distribution = self.simCore.distribution.createDistribution(
            random_dict, defect_onto.name
        )
        ...
        for normal_distribution in normal_distribution_list:
            ...
            defect_distribution_onto = self.simCore.distribution.createDistribution(
                normal_distribution["defect"], sub_defect_onto.name
            )
            repair_distribution_onto = self.simCore.distribution.createDistribution(
                normal_distribution["repair"], sub_defect_onto.name + "1"
            )
            defect_onto.has_for_sub_defect.append(sub_defect_onto)
            sub_defect_onto.has_for_sub_defect_distribution.append(
                defect_distribution_onto
            )
    ...
    def createSubDefect(self, defect_type):
        ...
        """
        sub_defect = self.simCore.central.sub_defect_class(
            Label.SubDefect.value + str(self.simCore.sub_defect_id)
        )
    ...
    def getNextDefectTime(self, defect_onto):
        ...
        if len(defect_onto.is_defect_of_machine) > 0:
            random_distribution = (
                defect_onto.has_for_defect_type_distribution.__getitem__(0)
            )
            random_value = self.simCore.distribution.getRandomTimefromOnto(
                random_distribution
            )
            sub_defect = defect_onto.has_for_sub_defect[random_value]
            distribution_onto = sub_defect.has_for_sub_defect_distribution.__getitem__(
                0
            )
            next_defect_time = (
                self.simCore.getCurrentTimestep()
                + self.simCore.distribution.getTimefromOnto(distribution_onto)
            )
