...
class ProdProcess:
    ...
    def createProdProces(self, process_id, machineInstance=None):
        ...
        prod_processInstance = self.simCore.central.prod_process_class(
            Label.ProdProcess.value + str(self.simCore.prod_process_id),
            namespace=self.simCore.onto,
        )
        prod_processInstance.is_prodprocess_of_process.append(
            self.simCore.onto[Label.Process.value + str(process_config["id"])]
        )
