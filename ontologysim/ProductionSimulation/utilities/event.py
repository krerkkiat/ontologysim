...
class EventUtilities:
    ...
    def calc_position_dict(self):
        ...
        """
        position_instances = self.simCore.onto.search(
    ...
    def calc_process_dict(self):
        ...
        """
        process_instances = self.simCore.onto.search(
            type=self.simCore.onto.search(iri="*#ProdProcess")
        )
    ...
    def transformEventLoggerOntoToList(self, event_onto):
        ...
        if Machine_Enum.Defect.value == event_onto.type_logger:
            machine_name = event_onto.has_for_machine_event_of_logger.__getitem__(
                0
            ).name
        elif Machine_Enum.SetUp.value == event_onto.type_logger:
            process_id = event_onto.has_for_process_event_of_logger.__getitem__(
                0
            ).process_id
            process_name = event_onto.has_for_process_event_of_logger.__getitem__(
                0
            ).name
            machine_name = self.process_dict[process_name]
            ...
            machine_name = self.process_dict[process_name]
            # TODO change if combine
            product_name = event_onto.has_for_product_event_of_logger.__getitem__(
                0
            ).name

        elif Queue_Enum.Change.value == event_onto.type_logger:
            position_name = event_onto.has_for_position_event_of_logger.__getitem__(
                0
            ).name
            product_name = event_onto.has_for_product_event_of_logger.__getitem__(
                0
            ...
            transport_name = event_onto.has_for_transport_event_of_logger.__getitem__(
                0
            ).name
            ...
            location_name = event_onto.has_for_location_event_of_logger.__getitem__(
                0
            ).name
        ...
        return [
            event_onto.name,
            event_onto.time_logger,
            event_onto.time_diff_logger,
            event_onto.type_logger,
            additional_type,
            product_name,
            position_name,
            position_info_name,
